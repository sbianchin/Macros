...
bool _Good_TOF1(int TOF1_Gap, vector<double> vec_int){
	...
	if(abs(Y_min-Y_max)<0.01){
		if(TOF1_Gap == 2){
			if((vec_int[0]>=0 && vec_int[0]<=X_max) && 
		   	   (vec_int[1]>=Y_min && vec_int[1]<=Y_max)) Good_TOF1 = true;			
		}
		else if(TOF1_Gap == 8){
			if((vec_int[0]>=X_min && vec_int[0]<=0) && 
		   	   (vec_int[1]>=Y_min && vec_int[1]<=Y_max)) Good_TOF1 = true;			
		}
	}
	...
	if(abs(Y_min-Y_max)<0.01){
		if(TOF1_Gap == 11){
			if((vec_int[0]>=X_min && vec_int[0]<=X_max) && 
		   	   (vec_int[1]>=0 && vec_int[1]<=Y_max)) Good_TOF1 = true;			
		}
		else if(TOF1_Gap == 5){
			if((vec_int[0]>=X_min && vec_int[0]<=X_max) && 
		   	   (vec_int[1]>=Y_min && vec_int[1]<=0)) Good_TOF1 = true;			
		}
	}
	else{	
		if((vec_int[0]>=X_min && vec_int[0]<=X_max) && 
...
vector<double> _good_intersect_with_circle(double a, double b, double R, int TOF1_gap){
	...
	dist1 = distance(intersect1[0], intersect1[1], TOF1_Xloc[TOF1_gap-1][2], TOF1_Yloc[TOF1_gap-1][2]);
	dist2 = distance(intersect2[0], intersect2[1], TOF1_Xloc[TOF1_gap-1][2], TOF1_Yloc[TOF1_gap-1][2]);

//// 8 -> 12 edge fibers
//// Modified Tie breaker conditions for TOF1 gap scoring  (16/01/2018)
...
#include "Plot_Event_Display_6.4.C"
...
void Event_Display(int Run_Number=3994, int ievt=0, int enable_cout=0, int Switch_Printout = 1,
  ...
  char Version[100] = "Version 6.4.1";  // TO MOVE
  ...
  //// GAP SCORING !
    ...
    for(int i=0; i<12; i++){
      if(TDC_TOF1U[i]>=TOF1U_TDC_min[i] && TDC_TOF1U[i]<=TOF1U_TDC_max[i]){
        //cout << "TOF1 Cond3 (TDC UP): " << i+1 << "  " << TDC_TOF1U[i] << endl;
        gap_counter[i]++;
      }
    }
    //cout << endl;
    ...
    for(int i=0; i<12; i++){
      if(TDC_TOF1D[i]>=TOF1D_TDC_min[i] && TDC_TOF1D[i]<=TOF1D_TDC_max[i]){
        //cout << "TOF1 Cond4 (TDC DOWN): " << i+1 << "  " << TDC_TOF1D[i] << endl;
        gap_counter[i]++;
      }
    }
    //cout << endl;

    //cout << "TOF1 TDC 12 UP " << TDC_TOF1U[11] << endl;
    //cout << "TOF1 TDC 12 DOWN: " << TDC_TOF1D[11] << endl;
    ...
    if(gap_counter[11]>0){
      ...
      if(TDC_TOF2AO[10]>=TOF2AO_TDC_min[10] && TDC_TOF2AO[10]<=TOF2AO_TDC_max[10]){
        gap_counter[11]++;
        //cout << "TOF2 Cond14 : " << 12 << "  ";
      }
      //cout << endl;
      ...
      if(TDC_TOF2AI[10]>=TOF2AI_TDC_min[10] && TDC_TOF2AI[10]<=TOF2AI_TDC_max[10]){
        gap_counter[11]++;
        //cout << "TOF2 Cond17 : " << 12 << "  ";
      }
      //cout << endl;
      ...
      if(TDC_TOF2BO[10]>=TOF2BO_TDC_min[10] && TDC_TOF2BO[10]<=TOF2BO_TDC_max[10]){
        gap_counter[11]++;
        //cout << "TOF2 Cond20 : " << 12 << "  ";
      }
      //cout << endl;
      ...
      if(TDC_TOF2BI[10]>=TOF2BI_TDC_min[10] && TDC_TOF2BI[10]<=TOF2BI_TDC_max[10]){
        gap_counter[11]++;
        //cout << "TOF2 Cond23 : " << 12 << "  ";
      }
      //cout << endl;
  ...
  //cout << "11: " << gap_counter[10] << endl;
  //cout << "12: " << gap_counter[11] << endl;
      ...
      if(TARGET_High_has_neighbours[i]){
        if(!k_stop_bar[i]){
          if(ADC_High_TARGET[i]>Angle_ADC_cut && has_TDC_hit[i]){
            ...
            vec_lepton_bars.push_back(TARGET_Rotated_index[i]);
  ...
  y2 = _perp_line(a_lepton_fit_3, vec_k_stop_coordinates[0], vec_k_stop_coordinates[1]);
  
  TLine *y2line = new TLine(-50,y2[0],50,y2[1]);    y2line->SetLineWidth(3);
  ...
  if(Switch_Printout != 0){
    ...
    if(tie_breaker){
      cout << endl;
      cout << "TIE BREAKER WAS USED !" << endl;
      cout << endl;
    }

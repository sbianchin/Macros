...
#include "Plot_Event_Display_6.3.C"
...
void Event_Display(int Run_Number=3994, int ievt=0, int enable_cout=0, int Switch_Printout = 1,
  ...
  char Version[100] = "Version 6.3.1";  // TO MOVE
  ...
  for(int i = 0; i<256; i++){
    for(int j=0; j<6; j++){
     //cout << "TEST : " << i << "   " << j << "   " << tdc_le_target[i][j] << endl;
     if(TDC_LG_max == -1){
...
*/
  ...
  // Fill KStop bars
  vector<int> vec_kaon_outlier;   vec_kaon_outlier.clear();
  bool test_kaon = false;

  for(int i = 0; i<256; i++){
    if(k_stop_bar[i]){
      test_kaon=false;
      for(int j=0; j<8; j++){
        if(k_stop_bar[TARGET_neighbours[i][j]]) test_kaon = true;       
        //if(!k_stop_bar[TARGET_neighbours[i][j]]) vec_kaon_outlier.push_back(i);
        //else vec_kaon_outlier.push_back(i);
      }
      if(!test_kaon) vec_kaon_outlier.push_back(i);
    }
  }
  ...
  for(unsigned int i=0; i<vec_kaon_outlier.size(); i++){
    vec_kaon_bars.erase(find(vec_kaon_bars.begin(), vec_kaon_bars.end(), vec_kaon_outlier[i]));
  }
...
/*  cout << endl;
  cout << "ALBANE " << endl;
  cout << "a_kaon_fit : " << a_fit_kaon << endl;
  cout << "b_kaon_fit : " << b_fit_kaon << endl;
  cout << "ChiS_kaon : " << Chis_kaon << endl;
  cout << "ndf_kaon : " << ndf_kaon << endl;
  cout << endl;
*/
  if(abs(a_fit_kaon)>100) TO_ROTATE_KAON = true;

/*  cout << endl;
  cout << "TO_ROTATE_KAON : " << TO_ROTATE_KAON << endl;
  cout << endl;
*/
  if(vec_xx_kaon.size()>=kaon_cluster_size){
    if(TO_ROTATE_KAON){
...
/*      cout << endl;
      cout << "ALBANE " << endl;
      cout << "a_fit_kaon : " << a_kaon_fit_rotate << endl;
      cout << "b_fit_kaon : " << b_kaon_fit_rotate << endl;
      cout << endl;
      cout << "A_FIT_KAON : " << -1/a_kaon_fit_rotate << endl;
      cout << "B_FIT_KAON : " << b_kaon_fit_rotate/a_kaon_fit_rotate << endl;
      cout << endl;
*/    }
    else{
      graph_kaon_rotate->Fit("gr_kaon_rotate","Q");
...
*/
  ...
  if(TO_ROTATE_KAON){
    ...
    //best_kaon_fit_rotate->SetLineStyle(2);
  }
  else{
  ...
  //   cout << endl;
  //   cout << "IS VERTICAL : " << vertical_kaon << endl;
  //   cout << endl;
...
*/
  ...
  if(vec_xx_lepton.size() > unsigned(weight_TOF1_fit3)){
...
*/
  ...
  //cout << endl;
  //cout << " ALBANE  " << endl;
  //cout << "empty :" << empty_lepton3 << endl;
  //cout << "angle_final : " << angle_final_guide << endl;
  //cout << endl;
...
*/
  ...
  if(vec_kaon_bars.size()>=kaon_cluster_size){
    if (vertical_kaon) {
      ...
      //if(gap_to_fit==6 || gap_to_fit==12){
      if(To_rotate){
        if(TO_ROTATE_KAON) vec_fit_lines_intersect = _2lines_intersect(a_fit_lepton_rotate, b_fit_lepton_rotate, A_FIT_KAON, B_FIT_KAON);
        else vec_fit_lines_intersect = _2lines_intersect(a_fit_lepton_rotate, b_fit_lepton_rotate, a_fit_kaon, b_fit_kaon);
...
/*  else if(abs(a_kaon_fit_rotate)>100){
    vec_fit_lines_intersect.push_back(999.99);
    vec_fit_lines_intersect.push_back(999.99);
...
/*  cout << endl;
  cout << "ALBANE ALBANE ALBANE " << endl;
  cout << "A_FIT_KAON : " << A_FIT_KAON << endl;
  cout << "a_fit_kaon_rotate : " << a_kaon_fit_rotate << endl;
  cout << "intersect_X : " <<  vec_fit_lines_intersect[0] << endl;
  cout << "intersect_Y : " <<  vec_fit_lines_intersect[1] << endl;
  cout << endl;
*/

/*  cout << endl;
  cout << "DEBUG " << endl;
  cout << "Line Intersect X : " << vec_fit_lines_intersect[0] << endl;
  cout << "Line Intersect Y : " << vec_fit_lines_intersect[1] << endl;
  cout << endl;
  cout << "Chis_kaon : " << Chis_kaon << endl;
  cout << "ndf_kaon : " << ndf_kaon << endl;
  cout << endl;
...
/*  cout << endl;
  cout << "SEBASTIEN" << endl;
  cout << vec_k_stop_coordinates[0] << endl;
  cout << vec_k_stop_coordinates[1] << endl;
  cout << endl;
*/
  i_kaon_bar = 999;
...
/*    TLine *TOF_line1 = new TLine(X_TOF1_line[0][0],Y_TOF1_line[0][0],X_TOF1_line[0][1],Y_TOF1_line[0][1]);
    TLine *TOF_line2 = new TLine(X_TOF1_line[1][0],Y_TOF1_line[1][0],X_TOF1_line[1][1],Y_TOF1_line[1][1]);
    TLine *TOF_line3 = new TLine(X_TOF1_line[2][0],Y_TOF1_line[2][0],X_TOF1_line[2][1],Y_TOF1_line[2][1]);
    TLine *TOF_line4 = new TLine(X_TOF1_line[3][0],Y_TOF1_line[3][0],X_TOF1_line[3][1],Y_TOF1_line[3][1]);
    TLine *TOF_line5 = new TLine(X_TOF1_line[4][0],Y_TOF1_line[4][0],X_TOF1_line[4][1],Y_TOF1_line[4][1]);
    TLine *TOF_line6 = new TLine(X_TOF1_line[5][0],Y_TOF1_line[5][0],X_TOF1_line[5][1],Y_TOF1_line[5][1]);
    TLine *TOF_line7 = new TLine(X_TOF1_line[6][0],Y_TOF1_line[6][0],X_TOF1_line[6][1],Y_TOF1_line[6][1]);
    TLine *TOF_line8 = new TLine(X_TOF1_line[7][0],Y_TOF1_line[7][0],X_TOF1_line[7][1],Y_TOF1_line[7][1]);
    TLine *TOF_line9 = new TLine(X_TOF1_line[8][0],Y_TOF1_line[8][0],X_TOF1_line[8][1],Y_TOF1_line[8][1]);    // TO MOVE
    TLine *TOF_line10 = new TLine(X_TOF1_line[9][0],Y_TOF1_line[9][0],X_TOF1_line[9][1],Y_TOF1_line[9][1]);
    TLine *TOF_line11 = new TLine(X_TOF1_line[10][0],Y_TOF1_line[10][0],X_TOF1_line[10][1],Y_TOF1_line[10][1]);
    TLine *TOF_line12 = new TLine(X_TOF1_line[11][0],Y_TOF1_line[11][0],X_TOF1_line[11][1],Y_TOF1_line[11][1]);
*/
...
*/
  ...
  if(print_output != 0){
    ...
    //cout << setw(1) << TO_ROTATE_KAON << endl;
    //if(bool_edge_bar==1) cout << setw(1) << setprecision(0) << 1 << " ";
    //else if(bool_edge_bar==0) cout << setw(1) << setprecision(0) << "FALSE" << " ";
    //else cout << setw(6) << setprecision(0) << "FALSE " << " ";
    cout << endl;
  }
  ...
  if(batch==1 && (batch_flag== 0 || batch_flag==2 || batch_flag==3)){ // {print_output
    ...
    fout << setw(1) << setprecision(0) << bool_edge_bar << " ";
    fout << setw(1) << setprecision(0) << Good_TOF1_intersect << " ";
    //if(has_edge_bars) fout << setw(6) << setprecision(0) << "TRUE " << " ";
    //else fout << setw(6) << setprecision(0) << "FALSE " << " ";
    fout << endl;
    fout.close();
  }

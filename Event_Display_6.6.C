...
void Event_Display(int Run_Number=3994, int ievt=0, int enable_cout=0, int Switch_Printout = 1,
  ...
  char Version[100] = "Version 6.6";  // TO MOVE
...
*/
  ...
  for(int i = 0; i<256; i++){
    if(k_stop_bar[i]){
      ...
      for(int j=0; j<8; j++){
        ...
        if(k_stop_bar[TARGET_neighbours[i][j]]){
          kaon_has_neighbours[i] = true;
  ...
  //vec_xx_pruning_area.push_back(vec_xx_kstop_final[0]);
  //vec_yy_pruning_area.push_back(vec_yy_kstop_final[0]);
  ...
  if(To_rotate){
    if(vec_kaon_bars.size()>0){
      ...
      if(counter_inside_polygon >= lepton_cluster_size){
        ...
        for(unsigned int i=0; i<vec_lepton_bars.size(); i++){
          ...
          //vec_xx_lepton_pruned_polygon.push_back(vec_xx_lepton_rotated[i]);
          vec_xx_lepton_pruned_polygon.push_back(Xloc[vec_lepton_bars[i]]);
          vec_ex_lepton_pruned_polygon.push_back(TARGET_Errors_X);
          //vec_yy_lepton_pruned_polygon.push_back(vec_yy_lepton_rotated[i]);
          vec_yy_lepton_pruned_polygon.push_back(Yloc[vec_lepton_bars[i]]);
          vec_ey_lepton_pruned_polygon.push_back(TARGET_Errors_Y);
        }     
      }
    ...
    if(vec_kaon_bars.size()>0){
      ...
      if(counter_inside_polygon >= lepton_cluster_size){
        //cout << "SSSSSSSIIIIIIZZZE : " << vec_xx_lepton.size() << endl;
        for(unsigned int i=0; i<vec_xx_lepton.size(); i++){
          //cout << i << "   " << vec_xx_lepton[i] << endl;
          if(gr_pruning_area->IsInside(vec_xx_lepton[i], vec_yy_lepton[i])==1){
            //cout << "DDD  " << i << "  " << vec_xx_lepton[i] << endl;
            vec_lepton_pruned_polygon.push_back(vec_lepton_bars[i]);
      ...
      //cout << "WE ARE HERE MOTHA !" << endl;
      //cout << vec_xx_lepton.size() << endl;
      //for(unsigned int i=0; i<vec_xx_lepton.size(); i++){
      for(unsigned int i=0; i<vec_lepton_bars.size(); i++){
        //cout << "FFF" << endl;
        vec_lepton_pruned_polygon.push_back(vec_lepton_bars[i]);
...
*/
  ...
  if(print_output != 0){
    ...
    cout << setw(7) << setprecision(2) << vec_yy_kstop_final[0] << " ";             // 22

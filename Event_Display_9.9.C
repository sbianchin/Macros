...
void Event_Display(int Run_Number=3994, int ievt=0, int enable_cout=0, int Switch_Printout = 1,
  ...
  ofstream fout3;
  char batch_output3[50];   // file that contains mismatched events (kekcc vs TRIUMF)

  if(batch==1 && (batch_flag== 0 || batch_flag==2 || batch_flag==3)){ // {print_output
    sprintf(batch_output, "Run%d_kekcc_500_8_output.txt", Run_Number);
    fout.open(batch_output, ios::app);

    sprintf(batch_output2, "Run_%d_Zlist.txt", Run_Number);
    fout2.open(batch_output2, ios::app);  

    sprintf(batch_output3, "Run%d_Mismatches_500_8.txt", Run_Number);
    fout3.open(batch_output3, ios::app);     
  }
  ...
  if(!Good_Event){
    if(batch==1){
      if(batch_flag==0 || batch_flag==2 || batch_flag==3){
        miss_counter++;
        vec_miss.push_back(ievt);
        fout << "  " << endl;
        fout3 << "MISS : " << miss_counter << endl;
        for(unsigned int i=0; i<vec_miss.size(); i++){
          fout3 << vec_miss[i] << "  ";
        }
        fout3 << endl;
      }
  ...
  double b_par_line = 999.;
  TLine *par_line;
  TLine *par_line_rotated;

  if(To_rotate){
    if(gap_to_fit_rotated!=3 || gap_to_fit_rotated!=9){
      b_par_line = vec_yy_kstop_0[0] - TOF1_line_slope[gap_to_fit_rotated-1]*vec_xx_kstop_0[0];
      par_line = new TLine(-50,-50*TOF1_line_slope[gap_to_fit_rotated-1]+b_par_line,50,50*TOF1_line_slope[gap_to_fit_rotated-1]+b_par_line);
      }
      else{
        par_line = new TLine(vec_xx_kstop_0[0],-50,vec_xx_kstop_0[0],50);     
      }
    }
  else{

    if(gap_to_fit!=3 || gap_to_fit!=9){
      b_par_line = vec_yy_kstop_0[0] - TOF1_line_slope[gap_to_fit-1]*vec_xx_kstop_0[0];
      par_line = new TLine(-50,-50*TOF1_line_slope[gap_to_fit-1]+b_par_line,50,50*TOF1_line_slope[gap_to_fit-1]+b_par_line);
    }
    else{
      par_line = new TLine(vec_xx_kstop_0[0],-50,vec_xx_kstop_0[0],50);     
    }
    ...
    par_line->SetLineWidth(2);
    par_line->SetLineColor(3);
  ...
  if(To_rotate){
    if(vec_kaon_bars_rotated.size()>0){
      if(gap_to_fit_rotated==1 || gap_to_fit_rotated==2 || gap_to_fit_rotated==10 || gap_to_fit_rotated==11 || gap_to_fit_rotated==12){ 
        for(unsigned int i=0; i<vec_lepton_bars_rotated.size(); i++){
          if(Yloc[vec_lepton_bars_rotated[i]] < Xloc[vec_lepton_bars_rotated[i]]*TOF1_line_slope[gap_to_fit_rotated-1]+b_par_line){
    ...
    if(vec_kaon_bars.size()>0){
      if(gap_to_fit==1 || gap_to_fit==2 || gap_to_fit==10 || gap_to_fit==11 || gap_to_fit==12){ 
        for(unsigned int i=0; i<vec_lepton_bars.size(); i++){
          if(Yloc[vec_lepton_bars[i]] < Xloc[vec_lepton_bars[i]]*TOF1_line_slope[gap_to_fit-1]+b_par_line){

...
vector<int> _get_TOF1_Gap(Int_t ADC_TOF1U[12], Int_t ADC_TOF1D[12], Int_t TDC_TOF1U[12], Int_t TDC_TOF1D[12],
  ...
  vec_output.push_back(gap_to_fit);

  for(int i=0; i<12; i++){
    vec_output.push_back(gap_counter[i]);
  }
...
vector<double> lineLeptonToTOF1(vector<int> vec_in, int TOF1_gap, double X_Kstop, double Y_Kstop){
  ...
  if(vec_in.size() == 0){
    ...
    vec_out.push_back(TOF1_Xloc[TOF1_gap-1][2]);
    vec_out.push_back(TOF1_Yloc[TOF1_gap-1][2]);
  }

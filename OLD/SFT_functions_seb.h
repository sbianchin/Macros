...
void Test(double phi=90., int layer=1, double p=1.){
  double Z1[16]={0.};
  double Z2[16]={0.};
  double Z3[18]={0.};
  double Z4[18]={0.};
  ...
  if(layer == 1){
    for(int N=1; N<16; N++){
        Z1[N] = z_offset_const[0] + layer_radius[0]*phi*tan(alpha[0])
               + (N-1)*ribbon_width[0]
  ...
  if(layer == 2){
    for(int N=1; N<16; N++){
        Z2[N] = z_offset_const[1] + layer_radius[1]*phi*tan(alpha[1]) 
               + (N-1)*ribbon_width[1]
  ...
  if(layer == 3){
    for(int N=1; N<18; N++){
      Z3[N] = z_offset_const[2] + layer_radius[2]*(2*PI-phi)*tan(alpha[2]) 
               + (N-1)*ribbon_width[2]
  ...
  if(layer == 4){
    for(int N=1; N<18; N++){
    Z4[N] = z_offset_const[3] + layer_radius[3]*(2*PI-phi)*tan(alpha[3]) 
               + (N-1)*ribbon_width[3]
...
void Test2(double phi=90., double p1=8., double p2=13., double p3=3., double p4=6.){
  double Z1[16]={0.};
  double Z2[16]={0.};
  double Z3[18]={0.};
  double Z4[18]={0.};
...
//  } 
  ...
  for(int i=1; i<16; i++){
  printf(" n = %2d  ||  %8.3f  %8.3f  %8.3f  %8.3f  %8.3f  %8.3f \n", i,Z1[i],Z2[i],Z3[i],Z4[i],DZ_12[i],DZ_34[i]);
  //cout << " n = " << i << "  ||   "  << Z1[i] << "  " << Z2[i] << "  " << Z3[i] << "  " << Z4[i] << endl;
  //cout << "" << endl;
  }

  for(int j=16; j<18; j++){
  printf(" n = %2d  ||  --------  --------  %8.3f  %8.3f  --------  %8.3f \n", j,Z3[j],Z4[j],DZ_34[j]);
...
vector<double> Layer_averaging(vector<int> p_fibers, bool *bad_sft_flag, int layer){
  ...
  //   p_centroids[i] = p_cluster_index_arr[i] + p_cluster_length_arr[i]/2.0 + 1;
  // }
    ...
    if(p1_p2_flag[*it] > 1 && p3_p4_flag[*it] > 1){
      if(z_avgs_store.empty()){
        ...
        for(vector<double>::iterator db = z_avgs_store.begin(); db != z_avgs_store.end(); db++){
          if(fabs(*db - z_avg/double(z_avg_count)) < 3){
            z_printed = true;
          }
        }
      }

...
void Batch_Read(Int_t Run_Number=5, Int_t Switch_Display=0, Int_t Switch_Zlist=0){ 
  ...
  char ascii[100];
  sprintf(ascii,"Keito_3405_mulist.txt");

  sprintf(output,"RUN_%d_Mu_DATA.txt",Run_Number);
  ofstream fout;
  fout.open(output);

  sprintf(output2,"Run_%d_Mu_BadEvents.txt",Run_Number);
  ofstream fout2;
  fout2.open(output2);
  ...
  ifstream file_to_open("Keito_3405_mulist.txt", ios::in);

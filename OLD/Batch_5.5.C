...
void Batch_5_5(int Run_Number=3994, int min_ievt=0, int max_ievt=1, int flag=1) {
  gROOT->SetBatch(1);
  ...
  if (flag == 0) {
    ...
    if(nentries>0){
      while (i <= max_ievt) {
        //if (i%10000==0 && i>0) {
        if (i%evt_counter==0 && i>0) {
          cout << i << " events processed!" << endl;
        }
        Event_Display_5_5(Run_Number, i, 0, 0, 0, 0, 1, 0);
        ++i;
      }
    }
    else{
    ...
    if(nentries>0){
      while (i <= max_ievt) {
        Event_Display_5_5(Run_Number, i, 0, 0, 0, 1, 1, 1);
        ++i;
      }
    }
    else{
    ...
    if(nentries>0){
      while(i<nentries) {
        ...
        Event_Display_5_5(Run_Number, i, 0, 0, 0, 0, 1, 2);
        ++i;
      }
    }
    else{
    ...
    if(nentries>0){
      while(fdat_read.good()){
        ...
        fdat_read >> Read_ievt;
        Event_Display_5_5(Run_Number, Read_ievt, 0, 0, 0, 0, 1, 3);
        read_counter++;
      }
    }
    else{

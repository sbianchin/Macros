...
void DrawTargetFrames(char ch_Title[100], char ch_ADC_cut_TARGET[100],
  ...
  tex_Title_ADC_High_TARGET = new TLatex(0.01759134,0.9295171,ch_Title);
...
void Subplot_1(char ch_Title[100], char ch_ADC_cut_TARGET[100],
  TLatex *tex_Legend_TARGET[36], TLatex *tex_event_TARGET,
  ...
  DrawTargetFrames(ch_Title, ch_ADC_cut_TARGET, tex_Legend_TARGET, tex_event_TARGET,
...
void Subplot_2(char ch_Title[100], char ch_ADC_cut_TARGET[100],
  TLatex *tex_Legend_TARGET[36], TLatex *tex_event_TARGET,
  Int_t ADC_High_TARGET[256], TMarker *marker_ADC_TARGET[256],
  Int_t ADC_Low_TARGET[256], Int_t TDC_min_TARGET,
  Int_t Switch, bool has_TDC_hit[256],
...
void Subplot_3(
  ...
  bool has_kaon_substitute, int kaon_sub,
  TLatex *tex_version2,
  TLine *TOF_line1, TLine *TOF_line2,
  TLine *TOF_line3, TLine *TOF_line4,
  TLine *TOF_line5, TLine *TOF_line6,
  ...
  if(has_kaon_substitute){
    marker_ADCL_TARGET[kaon_sub]->SetMarkerColor(6); 
    marker_ADCL_TARGET[kaon_sub]->Draw();
  }
...
void Subplot_5(TH2F *h_kaon_fit_final, TGaxis *A1,
  ...
  bool to_rotate_kaon, TLine *Fit_line) {
...
void Subplot_6(TH2F *h_final, TGaxis *A1,
  ...
  TGraph *gr_TOF1, TGraph *gr_TOF1_closest,
  TGraph *gr_int_TDC_TARGET, TGraph *gr_int_TDC_Gap_Fibers_SFT,
  TGraph *gr_kaon_stop, TGraph *gr_int_TOF1,
  TArrow *x_guide, TArrow *y_guide,
  TLatex *tex_Angle_guide, TLatex *tex_ChiS,
  TLatex *tex_Kstop_X, TLatex *tex_Kstop_Y,
  TLatex *x_sft, TLatex *y_sft, TLatex *x_target, TLatex *y_target,
  ...
  bool vertical_kaon, bool to_rotate_kaon, TLine *Fit_line) {
    ...
    //Fit_line->Draw("same");    
    if(to_rotate_kaon) Fit_line->Draw("same");
    gr_kaon_stop->Draw("P");
}
...
TH1F *h_ADC_L3_US, TH1F *h_ADC_L4_US, Int_t ADC_High_corr_max) {
  ...
  h_ADC_L1_DS->Draw("HIST");          h_ADC_L1_US->Draw("same, HIST");
  h_ADC_L2_DS->Draw("same, HIST");    h_ADC_L2_US->Draw("same, HIST");
  h_ADC_L3_DS->Draw("same, HIST");    h_ADC_L3_US->Draw("same, HIST");
  h_ADC_L4_DS->Draw("same, HIST");    h_ADC_L4_US->Draw("same, HIST");
...
/*void Subplot_pruning(TH2F *h_final, TGaxis *A1,
  TGaxis *A2, TGraph *gr_Target_Center,
  TEllipse *ell, TEllipse *ell_Target,
  TEllipse *ell_L1,
  int To_rotate,
...
/*void Subplot_pruning(TH2F *h_final, TGaxis *A1,
  ...
  TEllipse *ell_L1,
  int To_rotate,
  //TGraph *gr_lepton_pruning, TGraph *gr_lepton_pruning_rotate,
  TGraph *gr_lepton_pruning,
  TGraph *gr_kaon, TGraph *gr_kaon_rotate,
  TGraph *gr_TOF1, TGraph *gr_TOF1_closest,
  TGraph *gr_int_TDC_TARGET, TGraph *gr_int_TDC_Gap_Fibers_SFT,
  TGraph *gr_kaon_stop, TGraph *gr_int_TOF1){
*/
...
void Subplot_pruning(TH2F *h_final, TGaxis *A1,
  ...
  //bool vertical_kaon, bool to_rotate_kaon, TLine *Fit_line) {
    ...
    //if(To_rotate){
    //  gr_lepton_pruning_rotate->Draw("P");
    //}
    //else{
      gr_lepton_pruning->Draw("P");
    //}

    cerr << endl;
    cerr << "TEST FUNCTION " << endl;
    cerr << "TO ROTATE ? : " << To_rotate << endl;
    cerr << endl;

    if(To_rotate) gr_kaon_rotate->Draw("P");
    else gr_kaon->Draw("P");

...
void Batch_Job(int Run_Number=3994, int min_ievt=0, int max_ievt=1, int flag=1, int pruning_flag=0, bool FROM_FILE=false) {
  gROOT->SetBatch(1);
    ...
    if(nentries>0){
      if(FROM_FILE){
        ...
        //while(i<max_ievt && fdat_read.good()){
          ...
          Event_Display(Run_Number, Read_Run_Number, pruning_flag, 0, 0, 0, 0, 1, 0);
        ...
        while(i<=max_ievt){
          ...
          Event_Display(Run_Number, i, pruning_flag, 0, 0, 0, 0, 1, 0);
          if(Good_Event) tree.Fill();
          i++;
        }
        tree.Write();
    ...
    if(nentries>0){
      if(FROM_FILE){
        while(i<max_ievt && fdat_read.good()){
          ...
          if(counter%evt_counter==0 && i>0) cout << counter << " events processed!" << endl;
          Event_Display(Run_Number, Read_Run_Number, pruning_flag, 0, 0, 0, 1, 1, 1);
          //tree.Fill();
        }
        //tree.Write();
      }
        ...
        while(i<=max_ievt){
          if(i%evt_counter==0 && i>0) cout << i << " events processed!" << endl;
            Event_Display(Run_Number, i, pruning_flag, 0, 0, 0, 1, 1, 1);
            //if(Good_Event) tree.Fill();
          i++;
        }
        //tree.Write();
    ...
    if(nentries>0){
      if(FROM_FILE){
        while(i<=nentries && fdat_read.good()){
          ...
          if(counter%evt_counter==0 && i>0) cout << counter << " events processed!" << endl;
          Event_Display(Run_Number, Read_Run_Number, pruning_flag, 0, 0, 0, 0, 1, 2);
          if(Good_Event) tree.Fill();
          i = Read_Run_Number;
        }
        tree.Write();
        ...
        while(i<nentries){
          if(i%evt_counter==0 && i>0) cout << i << " events processed!" << endl;
          Event_Display(Run_Number, i, pruning_flag, 0, 0, 0, 0, 1, 2);
          if(Good_Event) tree.Fill();
          i++;
        }
        tree.Write();

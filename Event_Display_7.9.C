...
void Event_Display(int Run_Number=3994, int ievt=0, int enable_cout=0, int Switch_Printout = 1,
  ...
  char Version[100] = "Version 7.9";  // TO MOVE
...
*/
 ...
 bool tie_breaker = false;
  ...
  // NEW SELECTION (June 28 2018)
  for(int i = 0; i<256; i++){
    for(int j=0; j<8; j++){
      if((TARGET_neighbours[i][j] != -1 && ADC_High_TARGET[TARGET_neighbours[i][j]] > Angle_ADC_cut && has_TDC_hit[TARGET_neighbours[i][j]] && !k_stop_bar[TARGET_neighbours[i][j]]) ||
         (TARGET_neighbours[i][j] != -1 && ADC_High_TARGET[TARGET_neighbours[i][j]] <0 && ADC_Low_TARGET[TARGET_neighbours[i][j]]>0 && Switch==1 && !k_stop_bar[TARGET_neighbours[i][j]]) ||
         (TARGET_neighbours[i][j] != -1 && ADC_High_TARGET[TARGET_neighbours[i][j]] > Angle_ADC_cut && has_TDC_hit[TARGET_neighbours[i][j]] && k_stop_bar[TARGET_neighbours[i][j]]) || 
         (TARGET_neighbours[i][j] != -1 && ADC_High_TARGET[TARGET_neighbours[i][j]] <0 && ADC_Low_TARGET[TARGET_neighbours[i][j]]>0 && Switch==1 && k_stop_bar[TARGET_neighbours[i][j]])){
...
*/
  ...
  if(To_rotate_pre_prunning){   // TO CHECK (EMPTY LOOP)
    if(vec_kaon_bars.size()>0){
      for(unsigned int i=0; i<vec_lepton_bars_norepeat.size(); i++){
  ...
  vec_pruning_line = _pruning_line(vec_xx_kstop_0[0], vec_yy_kstop_0[0], gap_to_fit);
  ...
  for(int i = 0; i<weight_TOF1_fit1; i++){
    vec_xx_lepton.push_back(Gap[gap_to_fit-1][2][0]);
    vec_ex_lepton.push_back(TOF1_Errors_X[gap_to_fit-1][2]);

    vec_yy_lepton.push_back(Gap[gap_to_fit-1][2][1]);
    vec_ey_lepton.push_back(TOF1_Errors_Y[gap_to_fit-1][2]);
  }
  ...
  if(To_rotate_post_prunning){
    ...
    for(int i = 0; i<weight_TOF1_fit1; i++){
      vec_xx_lepton_rotate.push_back(Gap[gap_to_fit_rotated-1][2][0]);
      vec_ex_lepton_rotate.push_back(TOF1_Errors_X[gap_to_fit_rotated-1][2]);
      vec_yy_lepton_rotate.push_back(Gap[gap_to_fit_rotated-1][2][1]);
      vec_ey_lepton_rotate.push_back(TOF1_Errors_Y[gap_to_fit_rotated-1][2]);
    }
 ...
 if(vec_lepton_bars_pruned.size()>=lepton_cluster_size){
  ...
  else if(To_rotate_pre_prunning && To_rotate_post_prunning){
    temp = vec_lepton_bars_pruned;
    vec_lepton_bars_pruned.clear();
    for(unsigned int i=0; i<temp.size(); i++){
      vec_lepton_bars_pruned.push_back(temp[i]);
    }
    temp.clear();
  }
  ...
  if(To_rotate_pre_prunning || To_rotate_post_prunning){
    ...
    vec_ey_lepton.clear();
      ...
      for(int i = 0; i<weight_TOF1_fit1; i++){
        vec_xx_lepton.push_back(Gap[gap_to_fit_rotated-1][2][0]);
        vec_ex_lepton.push_back(TOF1_Errors_X[gap_to_fit_rotated-1][2]);
        vec_yy_lepton.push_back(Gap[gap_to_fit_rotated-1][2][1]);
        vec_ey_lepton.push_back(TOF1_Errors_Y[gap_to_fit_rotated-1][2]);
      }
  ...
  Delta_Y = sqrt(pow(TARGET_Errors_Y,2) + pow((cos(angle_final_guide*PI/180.)*R_TARGET*Delta_phi),2));
...
//////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  length_in_target = distance(vec_xx_kstop_final[0],vec_yy_kstop_final[0],
  ...
  if (counter_TDC_kaon > 0) Average_TDC_kaon = sum_TDC_kaon/counter_TDC_kaon;
    ...
    if(vec_leptons.size()<2 || (vec_leptons.size()==2 && angle_final_guide==999.)){
      ...
      line_test = new TLine(999., 999., 999., 999.);
    }
...
*/
    ...
    if(To_rotate_pre_prunning || To_rotate_post_prunning){  // TO MOVETArrow

...
#include "Plot_Event_Display_6.5.C"
...
void Event_Display(int Run_Number=3994, int ievt=0, int enable_cout=0, int Switch_Printout = 1,
  ...
  char Version[100] = "Version 6.5";  // TO MOVE
...
*/
  ...
  for(int i = 0; i<256; i++){
    if(k_stop_bar[i]){
      ...
      //cout << "Has_Neighbours : " << kaon_has_neighbours[i] << endl;
      //cout << endl;
      //vec_kaon_bars.push_back(i);
      //vec_kaon_bars_pruned.push_back(i);
    }
  ...
  if(To_rotate){
    ...
    //cerr << "LEPTON SIZE : " << vec_lepton_bars.size() << endl;
    //cerr << "LEPTON SIZE UNIQUE : " << vec_lepton_bars_norepeat.size() << endl;
    //cerr << "CLUSTER SIZE : " << lepton_cluster_size << endl;
    //cout << "//////// 2" << endl;
    if(vec_kaon_bars.size()>0){
      if(vec_lepton_bars.size()==0){
        ...
        //cout << "A lepton: " << a_fit_lepton_pr << endl;
        //cout << "B lepton: " << b_fit_lepton_pr << endl;
        //cout << "A kaon: " << a_fit_kaon_pr << endl;
        //cout << "B kaon: " << b_fit_kaon_pr << endl;
        //cout << "I AM HERE !" << endl;

        vec_intersect_pr = _2lines_intersect(a_fit_lepton_pr, b_fit_lepton_pr, a_fit_kaon_pr, b_fit_kaon_pr);

        //cout << "inter X = " << vec_intersect_pr[0] << endl;
        //cout << "inter Y = " << vec_intersect_pr[1] << endl;
  ...
  if(vec_intersect_pr.size()>0){
    if(vec_intersect_pr[0]*vec_intersect_pr[0]+
      ...
      //cout << endl;
      //cout << "/////////  INTERSECT PRUNED !" << "  " << ievt << endl;
      //cout << endl;
    }
  }

  //cout << endl;
  //cerr << vec_xx_kstop_0.size() << "  " <<  vec_yy_kstop_0.size() << endl;
  //cerr << "XX = " << vec_xx_kstop_0[0] << endl;
  //cerr << "YY = " << vec_yy_kstop_0[0] << endl;
  //cout << endl;


  graph_kstop_0 = new TGraph(vec_xx_kstop_0.size(),&vec_xx_kstop_0[0],&vec_yy_kstop_0[0]);

  if(Switch_Printout !=0)cout << "#########################################################################" << endl;
  ...
  if(To_rotate){
    ...
    for(unsigned int j=0; j<vec_lepton_bars_rotated.size(); j++){
      ...
      vec_lepton_bars_pruned_rotated.push_back(vec_lepton_bars_rotated[j]);

      vec_xx_lepton_pruned_rotated.push_back(Xloc[vec_lepton_bars_rotated[j]]);
      vec_ex_lepton_pruned_rotated.push_back(TARGET_Errors_X);

      vec_yy_lepton_pruned_rotated.push_back(Yloc[vec_lepton_bars_rotated[j]]);
      vec_ey_lepton_pruned_rotated.push_back(TARGET_Errors_Y);   
    }

    for(int k=0; k<weight_TOF1_fit1; k++){
 
      vec_xx_lepton_pruned_rotated.push_back(TOF1_Xloc[gap_to_fit_rotated-1][1]);
      vec_ex_lepton_pruned_rotated.push_back(TOF1_Errors_X[gap_to_fit_rotated-1][1]);
  
      vec_yy_lepton_pruned_rotated.push_back(TOF1_Yloc[gap_to_fit_rotated-1][1]);
      vec_ey_lepton_pruned_rotated.push_back(TOF1_Errors_Y[gap_to_fit_rotated-1][1]);
    }
    ...
    for(unsigned int j=0; j<vec_lepton_bars.size(); j++){
      ...
      vec_lepton_bars_pruned.push_back(vec_lepton_bars[j]);

      vec_xx_lepton_pruned.push_back(Xloc[vec_lepton_bars[j]]);
      vec_ex_lepton_pruned.push_back(TARGET_Errors_X);

      vec_yy_lepton_pruned.push_back(Yloc[vec_lepton_bars[j]]);
      vec_ey_lepton_pruned.push_back(TARGET_Errors_Y);   
    }

    for(int k=0; k<weight_TOF1_fit1; k++){
      vec_xx_lepton_pruned.push_back(TOF1_Xloc[gap_to_fit-1][1]);
      vec_ex_lepton_pruned.push_back(TOF1_Errors_X[gap_to_fit-1][1]);
  
      vec_yy_lepton_pruned.push_back(TOF1_Yloc[gap_to_fit-1][1]);
      vec_ey_lepton_pruned.push_back(TOF1_Errors_Y[gap_to_fit-1][1]);
    }
  ...
  if(To_rotate){
    for(unsigned int i=0; i<vec_lepton_bars_pruned_rotated.size(); i++){
      vec_lepton_bars_pruned_unique.push_back(vec_lepton_bars_pruned_rotated[i]);
  ...
  if(vec_lepton_bars_pruned_unique.size()<lepton_cluster_size){
    ...
    vec_lepton_bars_pruned_rotated.clear();
    ...
    if(To_rotate){
      ...
      for(unsigned int j=0; j<vec_lepton_bars_rotated_initial.size(); j++){
        ...
        vec_lepton_bars_pruned_rotated.push_back(vec_lepton_bars_rotated_initial[j]);

        vec_xx_lepton_pruned_rotated.push_back(Xloc[vec_lepton_bars_rotated_initial[j]]);
        vec_ex_lepton_pruned_rotated.push_back(TARGET_Errors_X);

        vec_yy_lepton_pruned_rotated.push_back(Yloc[vec_lepton_bars_rotated_initial[j]]);
        vec_ey_lepton_pruned_rotated.push_back(TARGET_Errors_Y);   
      }
      ...
      for(unsigned int j=0; j<vec_lepton_bars_initial.size(); j++){
        ...
        vec_lepton_bars_pruned.push_back(vec_lepton_bars_initial[j]);

        vec_xx_lepton_pruned.push_back(Xloc[vec_lepton_bars_initial[j]]);
        vec_ex_lepton_pruned.push_back(TARGET_Errors_X);

        vec_yy_lepton_pruned.push_back(Yloc[vec_lepton_bars_initial[j]]);
        vec_ey_lepton_pruned.push_back(TARGET_Errors_Y);   
      }
  ...
  //for(unsigned int i=0; i<vec_lepton_bars_initial.size(); i++){
  //  vec_lepton_bars_pruned.push_back(vec_lepton_bars_initial[i]);
  //}
  //}
  ...
  if(To_rotate){
    ...
    for(unsigned int i=0; i<vec_lepton_bars_pruned_rotated.size(); i++){
      ...
      vec_lepton_bars_pruned.push_back(TARGET_Rotated_index_inverse[vec_lepton_bars_pruned_rotated[i]]);

      //vec_xx_lepton_pruned.push_back(Xloc[TARGET_Rotated_index_inverse[vec_lepton_bars_rotated[i]]]);
      //vec_ex_lepton_pruned.push_back(TARGET_Errors_X);
      vec_xx_lepton_pruned.push_back(Xloc[TARGET_Rotated_index_inverse[vec_lepton_bars_pruned_rotated[i]]]);
      vec_ex_lepton_pruned.push_back(TARGET_Errors_X);

      //vec_yy_lepton_pruned.push_back(Yloc[TARGET_Rotated_index_inverse[vec_lepton_bars_rotated[i]]]);
      //vec_ey_lepton_pruned.push_back(TARGET_Errors_Y);
      vec_yy_lepton_pruned.push_back(Yloc[TARGET_Rotated_index_inverse[vec_lepton_bars_pruned_rotated[i]]]);
      vec_ey_lepton_pruned.push_back(TARGET_Errors_Y);
    }
  ...
  graph_lepton_pr3 = new TGraphErrors(vec_xx_lepton_pruned.size(),&vec_xx_lepton_pruned[0],&vec_yy_lepton_pruned[0],
                                     &vec_ex_lepton_pruned[0],&vec_ey_lepton_pruned[0]);
  ...
  TGraph *graph_kaon_pr4;
  TGraph *graph_lepton_pr4;

  vector<double> vec_xx_lepton_initial;       vec_xx_lepton_initial.clear();
  vector<double> vec_yy_lepton_initial;       vec_yy_lepton_initial.clear();
  vector<double> vec_ex_lepton_initial;       vec_ex_lepton_initial.clear();
  vector<double> vec_ey_lepton_initial;       vec_ey_lepton_initial.clear();

  vec_xx_lepton_pruned.clear();
  vec_yy_lepton_pruned.clear();
  vec_ex_lepton_pruned.clear();
  vec_ey_lepton_pruned.clear();
 

   for(unsigned int i=0; i<vec_lepton_bars_pruned.size(); i++){
    vec_xx_lepton_pruned.push_back(Xloc[vec_lepton_bars_pruned[i]]);
    vec_ex_lepton_pruned.push_back(TARGET_Errors_X);

    vec_yy_lepton_pruned.push_back(Yloc[vec_lepton_bars_pruned[i]]);
    vec_ey_lepton_pruned.push_back(TARGET_Errors_Y);
  }


  graph_lepton_pr4 = new TGraphErrors(vec_xx_lepton_pruned.size(),&vec_xx_lepton_pruned[0],&vec_yy_lepton_pruned[0],
                                     &vec_ex_lepton_pruned[0],&vec_ey_lepton_pruned[0]);


  graph_lepton_pr4->SetMarkerColor(2);
  graph_lepton_pr4->SetMarkerStyle(21);
  graph_lepton_pr4->SetMarkerSize(0.8);
  graph_lepton_pr4->GetYaxis()->SetTitle("Y title");
  graph_lepton_pr4->GetXaxis()->SetLimits(-50.,50.);
  graph_lepton_pr4->GetYaxis()->SetRangeUser(-50.,50.);
...
*/
    ...
    if(vertical_kaon){
      ...
      if(To_rotate){
        ...
        if(TO_ROTATE_KAON){
          ...
          vec_fit_lines_intersect = _2lines_intersect(a_lepton_fit_pr, b_lepton_fit_pr, a_fit_kaon, b_fit_kaon);
        }
      }
    }
  }
  ...
  //cout << "ALBANE : " << vec_fit_lines_intersect.size() << endl;
  //cout << vec_fit_lines_intersect[0] << "  " << vec_fit_lines_intersect[1] << endl;
  ...
  //gr_lepton_1 = new TGraphErrors(vec_lepton_bars_pruned.size(),&vec_xx_lepton_pruned[0],&vec_yy_lepton_pruned[0],
  //                              &vec_ex_lepton_pruned[0],&vec_ey_lepton_pruned[0]);

  //cout << vec_lepton_bars.size() << endl;
  //cout << vec_lepton_bars_norepeat.size() << endl;
  //cout << vec_lepton_bars_pruned.size() << endl;
  ...
  if(vec_xx_lepton_pruned.size()>=lepton_cluster_size){
...
*/
  ...
  if(Switch_Printout != 0){
    ...
    if(gap_to_fit==6 || gap_to_fit==12){
      ...
      cerr << endl;
      cerr << "gap_to_fit-1 : " << gap_to_fit-1 << endl;
...
*/
  ...
  if(print_output != 0){
    ...
    cout << setw(1) << setprecision(0) << bool_edge_bar << " ";
  ...
  if(batch==1 && (batch_flag== 0 || batch_flag==2 || batch_flag==3)){ // {print_output
    ...
    fout << setw(6) << setprecision(2) << vec_xx_int_TDC_TARGET[0] << " ";              // 15
    fout << setw(6) << setprecision(2) << vec_yy_int_TDC_TARGET[0] << " " ;             // 16
    ...
    fout << setw(1) << setprecision(0) << bool_edge_bar << " ";                         // 34

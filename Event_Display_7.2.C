// modified line 3164, 5221   -- if (vec_kaon_bars.size() < kaon_cluster_size){
// modify lines 3038,3039 for dX and dY calculation to use Delta_phi in RADIANS  MH -- Oct 14/17 
#ifndef __CINT__
#include <stdio.h>
#include <math.h>
#include <iomanip>
...
// using namespace CutsAndWindows;
...
void Event_Display(int Run_Number=3994, int ievt=0, int enable_cout=0, int Switch_Printout = 1,
  int Switch_Display = 1, int print_output = 1, int batch = 0, int batch_flag=0){


  //bool flag_pruning_line = true;
  ...
  char Version[100] = "Version 7.2";  // TO MOVE
...
/*   TFile f("tree1.root","update");
   TTree t1("t1","a test tree");

   t1.Branch("t_run_numer",&Run_Number,"t_run_number/I");
   t1.Branch("t_ievt",&ievt,"t_ievt/I");
   t1.Branch("t_evt_flag",&error_flag,"t_evt_flag/I");
*/
 ...
 bool tie_breaker = false;
  ...
  if(tof1_ties.size() > 1){  // only when there are more than 2 elements in tof1_ties!
    //cout << "****** TIE ALERT! -- Event : " << ievt << endl;
    tie_breaker = true;
  ...
  for(int k=0; k<12; k++){
    //cout << "SCORES : " << k+1 << "  " << gap_counter[k] << endl;  
  ...
  Gap_predicted = fun1(ADC_TOF1U, ADC_TOF1D, TDC_TOF1U, TDC_TOF1D,
                       ADC_TOF2AO, ADC_TOF2AI, ADC_TOF2BO, ADC_TOF2BI,
                       TDC_TOF2AO, TDC_TOF2AI, TDC_TOF2BO, TDC_TOF2BI);


/*  cout << "Gap to Fit : " << gap_to_fit << endl;
  lineLeptonToTOF1(gap_to_fit-1);
*/




  //cout << "COMPARISON : " << gap_to_fit << "  " << Gap_predicted << endl;

  if(gap_to_fit != Gap_predicted){
    cout << endl;
    cout << "#######  ALERT!  -- Event " << ievt << endl;
    fout << "#######  ALERT!  -- Event " << ievt << endl;
    cout << endl;
  }
  ...
  vector<int> vec_lepton_bars_pruned;             vec_lepton_bars_pruned.clear();
  vector<int> vec_lepton_bars_pruned_rotated;     vec_lepton_bars_pruned_rotated.clear();
  vector<int> vec_kaon_bars_pruned;               vec_kaon_bars_pruned.clear();
  vector<int> vec_lepton_bars_pruned_unique;      vec_lepton_bars_pruned_unique.clear();
  vector<int> vec_lepton_bars_initial;            vec_lepton_bars_initial.clear();
  vector<int> vec_lepton_bars_rotated_initial;    vec_lepton_bars_rotated_initial.clear();
...
*/
  ...
  // CHECK IF KAON BAR HAS NEIGHBOURS


  for(int i = 0; i<256; i++){
    if(k_stop_bar[i]){
      //cout << "k_stop_bar : " << i << endl;
      for(int j=0; j<8; j++){
        //cout << "Neighbours : " << TARGET_neighbours[i][j] << endl;
        if(k_stop_bar[TARGET_neighbours[i][j]]) kaon_has_neighbours[i] = true;
      }

      if(kaon_has_neighbours[i]){
        vec_kaon_bars.push_back(i);
        vec_kaon_bars_pruned.push_back(i);  
      }
  ...
  if(vec_kaon_bars.size()<kaon_cluster_size){
    for(int i = 0; i<256; i++){
      if(k_stop_bar[i] && !kaon_has_neighbours[i]){
        vec_kaon_bars.push_back(i);
        vec_kaon_bars_pruned.push_back(i);
      }
    }
  ...
  // Fill vectors with coordinates and errors
  for(unsigned int j=0; j<vec_kaon_bars.size(); j++){

    vec_xx_kaon.push_back(Xloc[vec_kaon_bars[j]]);
    vec_ex_kaon.push_back(TARGET_Errors_X);

    vec_yy_kaon.push_back(Yloc[vec_kaon_bars[j]]);
    vec_ey_kaon.push_back(TARGET_Errors_Y);   
  }



  if(vec_kaon_bars.size()==0 && has_kaon_sub){

    vec_kaon_bars.push_back(kaon_substitute);
    vec_xx_kaon.push_back(Xloc[kaon_substitute]);
    vec_ex_kaon.push_back(TARGET_Errors_X);

    vec_yy_kaon.push_back(Yloc[kaon_substitute]);
    vec_ey_kaon.push_back(TARGET_Errors_Y);       
  }
    ...
    if(TARGET_High_has_neighbours[i]){
      if(ADC_High_TARGET[i]>Angle_ADC_cut && has_TDC_hit[i]){
        if(!k_stop_bar[i]){
          ...
          vec_lepton_bars_norepeat.push_back(i);
          vec_lepton_bars_initial.push_back(i);
        }
        count++;
  ...
  for(unsigned int k=0; k<vec_lepton_bars_initial.size(); k++){
    vec_lepton_bars_rotated_initial.push_back(TARGET_Rotated_index[vec_lepton_bars_initial[k]]);
  }

  // Fill vectors with coordinates and errors 
  for(unsigned int j=0; j<vec_lepton_bars.size(); j++){
    if(vec_lepton_bars[j] != kaon_substitute){
      vec_xx_lepton.push_back(Xloc[vec_lepton_bars[j]]);
      vec_ex_lepton.push_back(TARGET_Errors_X);

      vec_yy_lepton.push_back(Yloc[vec_lepton_bars[j]]);
      vec_ey_lepton.push_back(TARGET_Errors_Y);
    }   
  }

  // Add TOF1 coordinates and errors
  for(int k=0; k<weight_TOF1_fit1; k++){
    vec_xx_lepton.push_back(TOF1_Xloc[gap_to_fit-1][2]);
    vec_ex_lepton.push_back(TOF1_Errors_X[gap_to_fit-1][2]);
  
    vec_yy_lepton.push_back(TOF1_Yloc[gap_to_fit-1][2]);
    vec_ey_lepton.push_back(TOF1_Errors_Y[gap_to_fit-1][2]);
  }

  //////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  //// CHECK IF NEED ROTATION  /////////////////////////////////////////////////////////////////////////////////////

  bool To_rotate_pre_prunning;
  To_rotate_pre_prunning = false;
  ...
  graph_kaon_0 = new TGraphErrors(vec_kaon_bars.size(),&vec_xx_kaon[0],&vec_yy_kaon[0],
                                 &vec_ey_kaon[0],&vec_ey_kaon[0]);

  double a_fit_kaon_0 = 999.;
  double b_fit_kaon_0 = 999.;
  double ChiS_kaon_0 = 999.;
  double ndf_kaon_0 = 999.;

  func_kaon_fit_0 = new TF1("gr_kaon_0", "pol1");

  graph_kaon_0->SetMarkerColor(4);
  graph_kaon_0->SetMarkerStyle(21);
  graph_kaon_0->SetMarkerSize(0.8);
  graph_kaon_0->GetYaxis()->SetTitle("Y title");
  graph_kaon_0->GetXaxis()->SetLimits(-50.,50.);
  graph_kaon_0->GetYaxis()->SetRangeUser(-50.,50.);

  if(vec_kaon_bars.size()>kaon_cluster_size){
    //graph_kaon_0->Fit("gr_kaon_0","QW");  // <--- OPTION "W" TO BE REMOVED IF TOO MANY ERROR MESSAGES !
    graph_kaon_0->Fit("gr_kaon_0","Q");

    func_kaon_fit_0 = graph_kaon_0->GetFunction("gr_kaon_0");
  ...
  //if(vec_kaon_bars.size()>kaon_cluster_size && abs(a_fit_kaon_0) > 10) To_rotate = true;
  if(vec_kaon_bars.size()>kaon_cluster_size && abs(a_fit_kaon_0) > 10) To_rotate_pre_prunning = true;

  /// LEPTONS
  TGraph *graph_lepton_0;
  TF1 *func_lepton_fit_0;

  graph_lepton_0 = new TGraphErrors(vec_xx_lepton.size(),&vec_xx_lepton[0],&vec_yy_lepton[0],
  ...
  //if(vec_lepton_bars_norepeat.size()>lepton_cluster_size && abs(a_fit_lepton_0) > 10) To_rotate = true;
  if(vec_lepton_bars_norepeat.size()>lepton_cluster_size && abs(a_fit_lepton_0) > 10) To_rotate_pre_prunning = true;

  vector<int> vec_lepton_bars_norepeat_rotated;
  vec_lepton_bars_norepeat_rotated.clear();

  //if(To_rotate){
  if(To_rotate_pre_prunning){
   for(unsigned int i=0; i<vec_lepton_bars_norepeat.size(); i++){
      vec_lepton_bars_norepeat_rotated.push_back(TARGET_Rotated_index[vec_lepton_bars_norepeat[i]]);     
    }
  }
  ...
  if(has_kaon_sub){
    //if(To_rotate){
    if(To_rotate_pre_prunning){
      vec_xx_kaon_substitute_rotated.push_back(Xloc[kaon_substitute]);
      vec_yy_kaon_substitute_rotated.push_back(Yloc[kaon_substitute]);
    }
    else{
      vec_xx_kaon_substitute.push_back(Xloc[kaon_substitute]);
      vec_yy_kaon_substitute.push_back(Yloc[kaon_substitute]);     
    }
  }

  if(kaon_substitute>0 && vec_kaon_bars.size()==0){
    vec_xx_kaon.push_back(Xloc[kaon_substitute]);
    vec_ex_kaon.push_back(TARGET_Errors_X);
  ...
  TGraph *graph_kaon_pr;            TGraph *graph_lepton_pr;
  TF1 *func_kaon_fit_pr;            TF1 *func_lepton_fit_pr;
  
  TGraph *graph_lepton_pr2;

  double a_fit_kaon_pr = 999.;    double a_fit_lepton_pr = 999.;
  double b_fit_kaon_pr = 999.;    double b_fit_lepton_pr = 999.;
  double ChiS_kaon_pr = 999.;     double ChiS_lepton_pr = 999.;
  double ndf_kaon_pr = 999.;      double ndf_lepton_pr = 999.;

  vector<int> vec_kaon_bars_rotated;     vec_kaon_bars_rotated.clear();
  vector<int> vec_lepton_bars_rotated;   vec_lepton_bars_rotated.clear();
  vector<double> vec_xx_kaon_rotated;       vec_xx_kaon_rotated.clear();
  vector<double> vec_yy_kaon_rotated;       vec_yy_kaon_rotated.clear();
  vector<double> vec_xx_lepton_rotated;     vec_xx_lepton_rotated.clear();
  vector<double> vec_yy_lepton_rotated;     vec_yy_lepton_rotated.clear();
  vector<double> vec_ex_kaon_rotated;       vec_ex_kaon_rotated.clear();
  vector<double> vec_ey_kaon_rotated;       vec_ey_kaon_rotated.clear();
  vector<double> vec_ex_lepton_rotated;     vec_ex_lepton_rotated.clear();
  vector<double> vec_ey_lepton_rotated;     vec_ey_lepton_rotated.clear();
  vector<double> vec_xx_lepton_pruned;      vec_xx_lepton_pruned.clear();
  vector<double> vec_yy_lepton_pruned;      vec_yy_lepton_pruned.clear();
  vector<double> vec_ex_lepton_pruned;      vec_ex_lepton_pruned.clear();
  vector<double> vec_ey_lepton_pruned;      vec_ey_lepton_pruned.clear();
  vector<double> vec_xx_lepton_pruned_rotated;      vec_xx_lepton_pruned_rotated.clear();
  vector<double> vec_yy_lepton_pruned_rotated;      vec_yy_lepton_pruned_rotated.clear();
  vector<double> vec_ex_lepton_pruned_rotated;      vec_ex_lepton_pruned_rotated.clear();
  vector<double> vec_ey_lepton_pruned_rotated;      vec_ey_lepton_pruned_rotated.clear();

  //if(To_rotate){
  if(To_rotate_pre_prunning){
    for(unsigned int i=0; i<vec_kaon_bars.size(); i++){
      vec_kaon_bars_rotated.push_back(TARGET_Rotated_index[vec_kaon_bars[i]]);
      vec_xx_kaon_rotated.push_back(Xloc[TARGET_Rotated_index[vec_kaon_bars[i]]]);
      vec_yy_kaon_rotated.push_back(Yloc[TARGET_Rotated_index[vec_kaon_bars[i]]]);

      vec_ex_kaon_rotated.push_back(TARGET_Errors_X);
      vec_ey_kaon_rotated.push_back(TARGET_Errors_Y);   
    }

    for(unsigned int i=0; i<vec_lepton_bars.size(); i++){
      if(vec_lepton_bars[i] != kaon_substitute){
        vec_lepton_bars_rotated.push_back(TARGET_Rotated_index[vec_lepton_bars[i]]);
        vec_xx_lepton_rotated.push_back(Xloc[TARGET_Rotated_index[vec_lepton_bars[i]]]);
        vec_yy_lepton_rotated.push_back(Yloc[TARGET_Rotated_index[vec_lepton_bars[i]]]);

        vec_ex_lepton_rotated.push_back(TARGET_Errors_X);
        vec_ey_lepton_rotated.push_back(TARGET_Errors_Y);   
      }
    }

    for(int k=0; k<weight_TOF1_fit1; k++){
      vec_xx_lepton_rotated.push_back(TOF1_Xloc[TOF1_rotated[gap_to_fit-1]][2]);
      vec_ex_lepton_rotated.push_back(TOF1_Errors_X[TOF1_rotated[gap_to_fit-1]][2]);
  
      vec_yy_lepton_rotated.push_back(TOF1_Yloc[TOF1_rotated[gap_to_fit-1]][2]);
      vec_ey_lepton_rotated.push_back(TOF1_Errors_Y[TOF1_rotated[gap_to_fit-1]][2]);
    }

    graph_kaon_pr = new TGraphErrors(vec_kaon_bars_rotated.size(),&vec_xx_kaon_rotated[0],&vec_yy_kaon_rotated[0],
                                      &vec_ex_kaon_rotated[0],&vec_ey_kaon_rotated[0]);

    graph_lepton_pr = new TGraphErrors(vec_xx_lepton_rotated.size(),&vec_xx_lepton_rotated[0],&vec_yy_lepton_rotated[0],
                                      &vec_ex_lepton_rotated[0],&vec_ey_lepton_rotated[0]);
  }
  else{
    graph_kaon_pr = graph_kaon_0;
    graph_lepton_pr = graph_lepton_0;
  }


  func_lepton_fit_pr = new TF1("gr_lepton_pr", "pol1");
  func_kaon_fit_pr = new TF1("gr_kaon_pr", "pol1");

  graph_kaon_pr->SetMarkerColor(4);
  graph_kaon_pr->SetMarkerStyle(21);
  graph_kaon_pr->SetMarkerSize(0.8);
  graph_kaon_pr->GetYaxis()->SetTitle("Y title");
  graph_kaon_pr->GetXaxis()->SetLimits(-50.,50.);
  graph_kaon_pr->GetYaxis()->SetRangeUser(-50.,50.);

  graph_lepton_pr->SetMarkerColor(2);
  graph_lepton_pr->SetMarkerStyle(21);
  graph_lepton_pr->SetMarkerSize(0.8);
  graph_lepton_pr->GetYaxis()->SetTitle("Y title");
  graph_lepton_pr->GetXaxis()->SetLimits(-50.,50.);
  graph_lepton_pr->GetYaxis()->SetRangeUser(-50.,50.);

  if(vec_kaon_bars.size()>kaon_cluster_size){

    //graph_kaon_pr->Fit("gr_kaon_pr","QW");  // <--- OPTION "W" TO BE REMOVED IF TOO MANY ERROR MESSAGES !
    graph_kaon_pr->Fit("gr_kaon_pr","Q");

    func_kaon_fit_pr = graph_kaon_pr->GetFunction("gr_kaon_pr");
    func_kaon_fit_pr->SetLineWidth(2);
    func_kaon_fit_pr->SetLineColor(4);

    a_fit_kaon_pr = func_kaon_fit_pr->GetParameter(1);
    b_fit_kaon_pr = func_kaon_fit_pr->GetParameter(0);
    ChiS_kaon_pr = func_kaon_fit_pr->GetChisquare();
    ndf_kaon_pr = func_kaon_fit_pr->GetNDF();
  }
  
  if(vec_lepton_bars_norepeat.size()>lepton_cluster_size){
    //graph_lepton_pr->Fit("gr_lepton_pr","QW");  // <--- OPTION "W" TO BE REMOVED IF TOO MANY ERROR MESSAGES !
    graph_lepton_pr->Fit("gr_lepton_pr","Q");

    func_lepton_fit_pr = graph_lepton_pr->GetFunction("gr_lepton_pr");
    func_lepton_fit_pr->SetLineWidth(2);
    func_lepton_fit_pr->SetLineColor(2);
  
    a_fit_lepton_pr = func_lepton_fit_pr->GetParameter(1);
    b_fit_lepton_pr = func_lepton_fit_pr->GetParameter(0);
    ChiS_lepton_pr = func_lepton_fit_pr->GetChisquare();
    ndf_lepton_pr = func_lepton_fit_pr->GetNDF();
  }
  ...
  if(To_rotate_pre_prunning){
    ...
    if(vec_kaon_bars.size()>0){
      for(unsigned int j=0; j<vec_kaon_bars.size(); j++){
        X_weights += ADC_Low_TARGET[vec_kaon_bars[j]]*Xloc[vec_kaon_bars_rotated[j]];
        Y_weights += ADC_Low_TARGET[vec_kaon_bars[j]]*Yloc[vec_kaon_bars_rotated[j]];
        total_energy += ADC_Low_TARGET[vec_kaon_bars[j]];
      }
    }
    ...
    if(vec_kaon_bars.size()>0 || total_energy!=0){
      ...
      vec_xx_kaon_centroid.push_back(X_weights/total_energy);
      vec_yy_kaon_centroid.push_back(Y_weights/total_energy);
    }
    else{
      vec_xx_kaon_centroid.push_back(0.);
      vec_yy_kaon_centroid.push_back(0.);
    }
  }
    ...
    if(vec_kaon_bars.size()>0){
      for(unsigned int j=0; j<vec_kaon_bars.size(); j++){
        X_weights += ADC_Low_TARGET[vec_kaon_bars[j]]*Xloc[vec_kaon_bars[j]];
        Y_weights += ADC_Low_TARGET[vec_kaon_bars[j]]*Yloc[vec_kaon_bars[j]];
        total_energy += ADC_Low_TARGET[vec_kaon_bars[j]];
      }
    }
  ...
  //   cout << endl;
  //   cout << "#########################################################################" << endl;
  //   cout << "CENTROID PRUNING" << endl;
  //   cerr << "X = " << vec_xx_kaon_centroid[0] << endl;
  //   cerr << "Y = " << vec_yy_kaon_centroid[0] << endl;
  //   cout << endl;
  //   cout << "#########################################################################" << endl;
  // }
  ...
  double kaon_ADC_LG_max = 0;
  int kaon_LG_max = 0;

  vector<double> vec_kstop_0;             vec_kstop_0.clear();
  vector<double> vec_xx_kstop_0;          vec_xx_kstop_0.clear();
  vector<double> vec_yy_kstop_0;          vec_yy_kstop_0.clear();
  vector<double> vec_xx_kstop_final;      vec_xx_kstop_final.clear();
  vector<double> vec_yy_kstop_final;      vec_yy_kstop_final.clear();

  vector<double> vec_intersect_pr;        vec_intersect_pr.clear();
  vector<double> vec_xx_intersect_pr;     vec_xx_intersect_pr.clear();
  vector<double> vec_yy_intersect_pr;     vec_yy_intersect_pr.clear();

  bool kstop_intersect = false;
  bool kstop_centroid = false;

  double dist_intersect_to_kaon = 999.;
  double smallest_kaon_distance = 999.;
  int closest_kaon = 999;

  TGraph *graph_kstop_0;

  //if(To_rotate){
  if(To_rotate_pre_prunning){
    if(vec_kaon_bars_rotated.size()>0){
      if(vec_lepton_bars_rotated.size()==0){
        vec_xx_kstop_0.push_back(vec_xx_kaon_centroid[0]);
        vec_yy_kstop_0.push_back(vec_yy_kaon_centroid[0]);
      }
      else if(vec_kaon_bars.size()<=kaon_cluster_size){
        for(unsigned int i=0; i<vec_kaon_bars.size(); i++){
          if(ADC_Low_TARGET[vec_kaon_bars[i]]+TARGET_ADC_Thr_LG_Offset>kaon_ADC_LG_max){
            kaon_ADC_LG_max = ADC_Low_TARGET[vec_kaon_bars[i]]+TARGET_ADC_Thr_LG_Offset;
            kaon_LG_max = vec_kaon_bars[i];
          }
        }
        vec_xx_kstop_0.push_back(Xloc[TARGET_Rotated_index[kaon_LG_max]]);
        vec_yy_kstop_0.push_back(Yloc[TARGET_Rotated_index[kaon_LG_max]]);
      }
      else{
        vec_intersect_pr = _2lines_intersect(a_fit_lepton_pr, b_fit_lepton_pr, a_fit_kaon_pr, b_fit_kaon_pr);

        for(unsigned int j=0; j<vec_kaon_bars.size(); j++){
          dist_intersect_to_kaon = sqrt(pow((Xloc[vec_kaon_bars_rotated[j]]-vec_intersect_pr[0]),2) + pow((Yloc[vec_kaon_bars_rotated[j]]-vec_intersect_pr[1]),2));
          if(dist_intersect_to_kaon < smallest_kaon_distance){
            smallest_kaon_distance = dist_intersect_to_kaon;
            closest_kaon = vec_kaon_bars_rotated[j];
          }
        }

        if(smallest_kaon_distance < DISTANCE_MAX_TO_KAON_PRUNED){
          vec_xx_kstop_0.push_back(vec_intersect_pr[0]);
          vec_yy_kstop_0.push_back(vec_intersect_pr[1]);
        }
        else{
          vec_xx_kstop_0.push_back(vec_xx_kaon_centroid[0]);
          vec_yy_kstop_0.push_back(vec_yy_kaon_centroid[0]);
      ...
      vec_xx_kstop_0.push_back(999.);
      vec_yy_kstop_0.push_back(999.);
      vec_xx_kstop_final.push_back(999.99);
      vec_yy_kstop_final.push_back(999.99);
    }
  }

  else{
    if(vec_kaon_bars.size()>0){
      if(vec_lepton_bars.size()==0){
        vec_xx_kstop_0.push_back(vec_xx_kaon_centroid[0]);
        vec_yy_kstop_0.push_back(vec_yy_kaon_centroid[0]);
        kstop_centroid = true;
      }
        ...
        vec_xx_kstop_0.push_back(Xloc[kaon_LG_max]);
        vec_yy_kstop_0.push_back(Yloc[kaon_LG_max]);
       kstop_centroid = true;
      }
      else{
        vec_intersect_pr = _2lines_intersect(a_fit_lepton_pr, b_fit_lepton_pr, a_fit_kaon_pr, b_fit_kaon_pr);
        ...
        for(unsigned int j=0; j<vec_kaon_bars.size(); j++){
          dist_intersect_to_kaon = sqrt(pow((Xloc[vec_kaon_bars[j]]-vec_intersect_pr[0]),2) + pow((Yloc[vec_kaon_bars[j]]-vec_intersect_pr[1]),2));
          if(dist_intersect_to_kaon < smallest_kaon_distance){
            smallest_kaon_distance = dist_intersect_to_kaon;
        ...
        //cerr << "DISTANCE : " << smallest_kaon_distance << endl;
        //cerr << "CLOSEST KAON : " << closest_kaon << endl;

        //cout << "DIST BBBB" << endl;
        //cerr << smallest_kaon_distance << endl;      // !! TO BE FIXED !!
        if(smallest_kaon_distance < DISTANCE_MAX_TO_KAON_PRUNED){
          vec_xx_kstop_0.push_back(vec_intersect_pr[0]);
          vec_yy_kstop_0.push_back(vec_intersect_pr[1]);
        }
  ...
  if(vec_intersect_pr.size()>0){
    if(vec_intersect_pr[0]*vec_intersect_pr[0]+
      vec_intersect_pr[1]*vec_intersect_pr[1] > R_Cut*R_Cut){
    }
  }

  graph_kstop_0 = new TGraph(vec_xx_kstop_0.size(),&vec_xx_kstop_0[0],&vec_yy_kstop_0[0]);

  ///////////////////////////////////////////////////////////////////////////////////////////////////////////
  //// POLYGONAL PRUNING

  /// Determine Gap_minus and Gap_plus;
 /* int TOF1_Gap_minus;
  int TOF1_Gap_plus;

  if(To_rotate){
    if(gap_to_fit_rotated-1==0){
      TOF1_Gap_minus = 11;
      TOF1_Gap_plus = 1;
    }
    else if(gap_to_fit_rotated-1==11){
      TOF1_Gap_minus = 10;
      TOF1_Gap_plus = 0;
    }
    else{
      TOF1_Gap_minus = (gap_to_fit_rotated-1)-1;
      TOF1_Gap_plus = (gap_to_fit_rotated-1)+1;
    }
  }
  else{ 
    if(gap_to_fit-1==0){
    ...
    else if(gap_to_fit-1==11){
      TOF1_Gap_minus = 10;
      TOF1_Gap_plus = 0;
    }
    else{
      TOF1_Gap_minus = (gap_to_fit-1)-1;
      TOF1_Gap_plus = (gap_to_fit-1)+1;
    }
  }
  */
  ...
  //if(To_rotate) TOF1_neighbours = _neighbour_TOF1(gap_to_fit_rotated);
  if(To_rotate_pre_prunning) TOF1_neighbours = _neighbour_TOF1(gap_to_fit_rotated);
  else TOF1_neighbours = _neighbour_TOF1(gap_to_fit);

  vector<double> vec_xx_pruning_area;   vec_xx_pruning_area.clear();
  vector<double> vec_yy_pruning_area;   vec_yy_pruning_area.clear();

  //NEW
  vec_xx_pruning_area = _pruning_area(vec_xx_kstop_0[0], TOF1_Xloc[TOF1_neighbours[0]][2], TOF1_Xloc[TOF1_neighbours[1]][2]);
  vec_yy_pruning_area = _pruning_area(vec_yy_kstop_0[0], TOF1_Yloc[TOF1_neighbours[0]][2], TOF1_Yloc[TOF1_neighbours[1]][2]);
  //vec_xx_pruning_area = _pruning_area(vec_xx_kstop_0[0], TOF1_Xloc[TOF1_Gap_minus][1], TOF1_Xloc[TOF1_Gap_plus][1]);
  //vec_yy_pruning_area = _pruning_area(vec_yy_kstop_0[0], TOF1_Yloc[TOF1_Gap_minus][1], TOF1_Yloc[TOF1_Gap_plus][1]);


/*  vec_xx_pruning_area.push_back(vec_xx_kstop_0[0]);
  vec_yy_pruning_area.push_back(vec_yy_kstop_0[0]);
  
  vec_xx_pruning_area.push_back(TOF1_Xloc[TOF1_Gap_minus][1]);
  vec_yy_pruning_area.push_back(TOF1_Yloc[TOF1_Gap_minus][1]);
  
  vec_xx_pruning_area.push_back(TOF1_Xloc[TOF1_Gap_plus][1]);
  vec_yy_pruning_area.push_back(TOF1_Yloc[TOF1_Gap_plus][1]);
...
*/
  ...
  //vector<double> vec_lepton_pruned;   vec_lepton_pruned.clear();
  //vector<double> vec_lepton_TBR;      vec_lepton_TBR.clear();  // TBR = To Be Removed

  //vector<double> vec_lepton_pruned_rotated;   vec_lepton_pruned_rotated.clear();
  //vector<double> vec_lepton_TBR_rotated;      vec_lepton_TBR_rotated.clear();  // TBR = To Be Removed

  vector<double> vec_xx_lepton_pruned_polygon_plot;    vec_xx_lepton_pruned_polygon_plot.clear();
  vector<double> vec_yy_lepton_pruned_polygon_plot;    vec_yy_lepton_pruned_polygon_plot.clear();

  vector<int> vec_lepton_pruned_polygon;            vec_lepton_pruned_polygon.clear();
  vector<double> vec_xx_lepton_pruned_polygon;      vec_xx_lepton_pruned_polygon.clear();
  vector<double> vec_yy_lepton_pruned_polygon;      vec_yy_lepton_pruned_polygon.clear();

  vector<double> vec_ex_lepton_pruned_polygon;      vec_ex_lepton_pruned_polygon.clear();
  vector<double> vec_ey_lepton_pruned_polygon;      vec_ey_lepton_pruned_polygon.clear();

  unsigned int counter_inside_polygon=0;
  
  //cout << "SIZES : " << endl;
  //cout << "NO REPEAT : " << vec_lepton_bars_norepeat.size() << endl;
  //cout << "        : " << vec_lepton_bars.size() << endl;
  //cout << endl;

  //cout << "SIZE KAON SIZE: " << vec_kaon_bars.size() << endl;

  //for(unsigned int j=0; j<vec_lepton_bars.size(); j++){
    //cout << "BARS   :  " << j << "  " << vec_lepton_bars[j] << endl;
  //}
  ...
  vec_pruned.clear();
  //cout << "TEST !" << endl;
  //vec_pruned = _Triangular_pruning(To_rotate, gr_pruning_area, vec_lepton_bars_norepeat, vec_kaon_bars);
  vec_pruned = _Triangular_pruning(To_rotate_pre_prunning, gr_pruning_area, vec_lepton_bars_norepeat, vec_kaon_bars);
  ...
  for(unsigned int i=0; i<vec_pruned.size(); i++){
    //if(To_rotate){
    if(To_rotate_pre_prunning){
      if(vec_pruned[i] != TARGET_Rotated_index[kaon_substitute]){
        vec_xx_lepton_pruned_polygon_plot.push_back(Xloc[vec_pruned[i]]);
        vec_yy_lepton_pruned_polygon_plot.push_back(Yloc[vec_pruned[i]]);
      }
    }
    else{
      if(vec_pruned[i] != kaon_substitute){
        vec_xx_lepton_pruned_polygon_plot.push_back(Xloc[vec_pruned[i]]);
        vec_yy_lepton_pruned_polygon_plot.push_back(Yloc[vec_pruned[i]]);
      }
    }
...
/*  for(unsigned int i=0; i<vec_pruned.size(); i++){
    cout << "### " << vec_pruned[i] << "   " << TARGET_Rotated_index_inverse[vec_pruned[i]] << endl;
  }
*/
  //for(unsigned int j=0; j<vec_lepton_bars_norepeat.size(); j++){
  for(unsigned int j=0; j<vec_pruned.size(); j++){
    //if(To_rotate){
    if(To_rotate_pre_prunning){

      if(vec_pruned[j] != TARGET_Rotated_index[kaon_substitute]){
        //vec_xx_lepton_pruned_polygon.push_back(Xloc[TARGET_Rotated_index[vec_lepton_bars_norepeat[j]]]);
        //vec_yy_lepton_pruned_polygon.push_back(Yloc[TARGET_Rotated_index[vec_lepton_bars_norepeat[j]]]);
        //vec_xx_lepton_pruned_polygon.push_back(Xloc[vec_lepton_bars_norepeat[j]]);
        //vec_yy_lepton_pruned_polygon.push_back(Yloc[vec_lepton_bars_norepeat[j]]);
        vec_lepton_pruned_polygon.push_back(vec_pruned[j]);    
        vec_xx_lepton_pruned_polygon.push_back(Xloc[TARGET_Rotated_index_inverse[vec_pruned[j]]]);
        vec_yy_lepton_pruned_polygon.push_back(Yloc[TARGET_Rotated_index_inverse[vec_pruned[j]]]);
        vec_ex_lepton_pruned_polygon.push_back(TARGET_Errors_X);
        vec_ey_lepton_pruned_polygon.push_back(TARGET_Errors_Y); 
      }
    }
    else{
      if(vec_pruned[j] != kaon_substitute){
        //vec_xx_lepton_pruned_polygon.push_back(Xloc[vec_lepton_bars_norepeat[j]]);
        //vec_yy_lepton_pruned_polygon.push_back(Yloc[vec_lepton_bars_norepeat[j]]); 
        vec_lepton_pruned_polygon.push_back(vec_pruned[j]);    
        vec_xx_lepton_pruned_polygon.push_back(Xloc[vec_pruned[j]]);
        vec_yy_lepton_pruned_polygon.push_back(Yloc[vec_pruned[j]]);
        vec_ex_lepton_pruned_polygon.push_back(TARGET_Errors_X);
        vec_ey_lepton_pruned_polygon.push_back(TARGET_Errors_Y); 
      }     
    }
  ...
  if(To_rotate_pre_prunning){
    if(vec_kaon_bars.size()>0){
      for(unsigned int i=0; i<vec_lepton_bars_norepeat.size(); i++){
        if(gr_pruning_area->IsInside(Xloc[TARGET_Rotated_index[vec_lepton_bars_norepeat[i]]], 
                                   Yloc[TARGET_Rotated_index[vec_lepton_bars_norepeat[i]]])==1){
          counter_inside_polygon++;
        }
      }

      if(counter_inside_polygon >= lepton_cluster_size){
        for(unsigned int i=0; i<vec_lepton_bars.size(); i++){
          if(gr_pruning_area->IsInside(vec_xx_lepton_rotated[i], vec_yy_lepton_rotated[i])==1){
            //vec_lepton_pruned_polygon.push_back(vec_lepton_bars[i]);

            ////vec_xx_lepton_pruned_polygon_plot.push_back(vec_xx_lepton_rotated[i]);
            ////vec_yy_lepton_pruned_polygon_plot.push_back(vec_yy_lepton_rotated[i]);

            ////vec_xx_lepton_pruned_polygon.push_back(Xloc[vec_lepton_bars[i]]);
            ////vec_ex_lepton_pruned_polygon.push_back(TARGET_Errors_X);
            ///vec_yy_lepton_pruned_polygon.push_back(Yloc[vec_lepton_bars[i]]);
            ////vec_ey_lepton_pruned_polygon.push_back(TARGET_Errors_Y);
          }
        }
      }
    ...
    if(vec_kaon_bars.size()>0){
      ...
      for(unsigned int i=0; i<vec_lepton_bars_norepeat.size(); i++){
        if(gr_pruning_area->IsInside(Xloc[vec_lepton_bars_norepeat[i]], 
                                   Yloc[vec_lepton_bars_norepeat[i]])==1){
          counter_inside_polygon++;
        }
      }
    
      if(counter_inside_polygon >= lepton_cluster_size){
        for(unsigned int i=0; i<vec_xx_lepton.size(); i++){
          if(gr_pruning_area->IsInside(vec_xx_lepton[i], vec_yy_lepton[i])==1){
            //vec_lepton_pruned_polygon.push_back(vec_lepton_bars[i]);

            ////vec_xx_lepton_pruned_polygon_plot.push_back(vec_xx_lepton[i]);
            ////vec_yy_lepton_pruned_polygon_plot.push_back(vec_yy_lepton[i]);

            ////vec_xx_lepton_pruned_polygon.push_back(vec_xx_lepton[i]);
            ////vec_ex_lepton_pruned_polygon.push_back(TARGET_Errors_X);
            ////vec_yy_lepton_pruned_polygon.push_back(vec_yy_lepton[i]);
            ////vec_ey_lepton_pruned_polygon.push_back(TARGET_Errors_Y);
          }
        }
      }
        ...
        for(unsigned int i=0; i<vec_lepton_bars.size(); i++){
          ...
          ////vec_xx_lepton_pruned_polygon_plot.push_back(Xloc[vec_lepton_bars[i]]);
          ////vec_yy_lepton_pruned_polygon_plot.push_back(Yloc[vec_lepton_bars[i]]);
  ...
  //cout << "COUNTER INSIDE POLYGON : " << counter_inside_polygon << endl; 

  //cout << "PFFF :: " << vec_xx_lepton_pruned_polygon_plot.size() << "  "
  //                   << vec_yy_lepton_pruned_polygon_plot.size() << endl;

  TGraph *gr_pruning_polygon_plot;
  gr_pruning_polygon_plot = new TGraph(vec_xx_lepton_pruned_polygon_plot.size(), &vec_xx_lepton_pruned_polygon_plot[0], &vec_yy_lepton_pruned_polygon_plot[0]);
  gr_pruning_polygon_plot->SetMarkerStyle(21);
  gr_pruning_polygon_plot->SetMarkerSize(0.8);
  gr_pruning_polygon_plot->SetMarkerColor(3);

  TGraph *gr_pruning_polygon;
  gr_pruning_polygon = new TGraphErrors(vec_xx_lepton_pruned_polygon.size(), 
                                        &vec_xx_lepton_pruned_polygon[0], &vec_yy_lepton_pruned_polygon[0],
                                        &vec_ex_lepton_pruned_polygon[0], &vec_ey_lepton_pruned_polygon[0]);
  gr_pruning_polygon->SetMarkerStyle(21);
  gr_pruning_polygon->SetMarkerSize(0.8);
  gr_pruning_polygon->SetMarkerColor(2);


/*  if(){
      for(unsigned int i=0; i<vec_lepton_bars.size(); i++){
    if(gr_pruning_area->IsInside(vec_xx_lepton[i], vec_yy_lepton[i])==1){
      vec_pruned_lepton_bars.push_back(vec_lepton_bars[i]);
      vec_xx_pruned_lepton_bars.push_back(Xloc[vec_lepton_bars[i]]);
      vec_yy_pruned_lepton_bars.push_back(Yloc[vec_lepton_bars[i]]);
    }
  }
...
*/
  ...
  ////////////////////////////////////////////////////////////////////////////////////////////////
  /// DETERMINE CUT LINE
  // Calculation of b  ( b = Y - aX )

  double b_pruning_line = 999.;
  //TLine *pruning_line_rotated;

  double x_pruning_min = 999.;
  double x_pruning_max = 999.;
  double y_pruning_min = 999.;
  double y_pruning_max = 999.;

  //if(To_rotate){
  if(To_rotate_pre_prunning){
    if(gap_to_fit_rotated!=3 || gap_to_fit_rotated!=9){
      b_pruning_line = vec_yy_kstop_0[0] - TOF1_line_slope[gap_to_fit_rotated-1]*vec_xx_kstop_0[0];
      x_pruning_min = -50.;
      x_pruning_max = 50.;
      y_pruning_min = x_pruning_min*TOF1_line_slope[gap_to_fit_rotated-1]+b_pruning_line;
      y_pruning_max = x_pruning_max*TOF1_line_slope[gap_to_fit_rotated-1]+b_pruning_line;
      //pruning_line = new TLine(-50,-50*TOF1_line_slope[gap_to_fit_rotated-1]+b_pruning_line,50,50*TOF1_line_slope[gap_to_fit_rotated-1]+b_pruning_line);
      //pruning_line = new TLine(x_pruning_min,y_pruning_min,x_pruning_max,y_pruning_max);
      }
      else{
      x_pruning_min = vec_xx_kstop_0[0];
      x_pruning_max = vec_xx_kstop_0[0];
      y_pruning_min = -50.;
      y_pruning_max = 50.;      
      //pruning_line = new TLine(vec_xx_kstop_0[0],-50,vec_xx_kstop_0[0],50);     
      //pruning_line = new TLine(x_pruning_min,y_pruning_min,x_pruning_max,y_pruning_max);     
      }
    }
  else{

    if(gap_to_fit!=3 || gap_to_fit!=9){
      b_pruning_line = vec_yy_kstop_0[0] - TOF1_line_slope[gap_to_fit-1]*vec_xx_kstop_0[0];     
      x_pruning_min = -50.;
      x_pruning_max = 50.;
      y_pruning_min = x_pruning_min*TOF1_line_slope[gap_to_fit-1]+b_pruning_line;
      y_pruning_max = x_pruning_max*TOF1_line_slope[gap_to_fit-1]+b_pruning_line;
      //pruning_line = new TLine(-50,-50*TOF1_line_slope[gap_to_fit-1]+b_pruning_line,50,50*TOF1_line_slope[gap_to_fit-1]+b_pruning_line);
      //pruning_line = new TLine(x_pruning_min,y_pruning_min,x_pruning_max,y_pruning_max);
    }
    else{
      x_pruning_min = vec_xx_kstop_0[0];
  ...
  //if(To_rotate) vec_pruning_line = _pruning_line(vec_xx_kstop_0[0], vec_yy_kstop_0[0], gap_to_fit_rotated);
  if(To_rotate_pre_prunning) vec_pruning_line = _pruning_line(vec_xx_kstop_0[0], vec_yy_kstop_0[0], gap_to_fit_rotated);
  else vec_pruning_line = _pruning_line(vec_xx_kstop_0[0], vec_yy_kstop_0[0], gap_to_fit);

  TLine *pruning_line;
  pruning_line = new TLine(vec_pruning_line[0], vec_pruning_line[1], vec_pruning_line[2], vec_pruning_line[3]);
  pruning_line->SetLineWidth(2);
  pruning_line->SetLineColor(3);
  ...
  vector<int> vec_lepton_pruned_line;          vec_lepton_pruned_line.clear();
  vector<double> vec_xx_lepton_pruned_line;       vec_xx_lepton_pruned_line.clear();
  vector<double> vec_yy_lepton_pruned_line;       vec_yy_lepton_pruned_line.clear();
  vector<double> vec_ex_lepton_pruned_line;       vec_ex_lepton_pruned_line.clear();
  vector<double> vec_ey_lepton_pruned_line;       vec_ey_lepton_pruned_line.clear();


  vector<int> vec_lepton_pruned_line_rotated;      vec_lepton_pruned_line_rotated.clear();
  vector<double> vec_xx_lepton_pruned_line_rotated;   vec_xx_lepton_pruned_line_rotated.clear();
  vector<double> vec_yy_lepton_pruned_line_rotated;   vec_yy_lepton_pruned_line_rotated.clear();
  vector<double> vec_ex_lepton_pruned_line_rotated;   vec_ex_lepton_pruned_line_rotated.clear();
  vector<double> vec_ey_lepton_pruned_line_rotated;   vec_ey_lepton_pruned_line_rotated.clear();

  vector<double> vec_xx_lepton_pruned_line_plot;   vec_xx_lepton_pruned_line_plot.clear();
  vector<double> vec_yy_lepton_pruned_line_plot;   vec_yy_lepton_pruned_line_plot.clear();

  ///NEW
  //vec_lepton_bars_norepeat

  //cout << "TO REPEAT : " << To_rotate << endl;


  //if(To_rotate) _Line_pruning(vec_lepton_bars_norepeat_rotated);
  //else _Line_pruning(vec_lepton_bars_norepeat);
  ...
  if(To_rotate_pre_prunning){
    if(vec_kaon_bars_rotated.size()>0){
      if(gap_to_fit_rotated==1 || gap_to_fit_rotated==2 || gap_to_fit_rotated==10 || gap_to_fit_rotated==11 || gap_to_fit_rotated==12){ 
        for(unsigned int i=0; i<vec_lepton_bars_rotated.size(); i++){
          if(Yloc[vec_lepton_bars_rotated[i]] < Xloc[vec_lepton_bars_rotated[i]]*TOF1_line_slope[gap_to_fit_rotated-1]+b_pruning_line){
            vec_lepton_TBR_rotated.push_back(vec_lepton_bars_rotated[i]);
          }
        } 
      }
      else if(gap_to_fit_rotated==4 || gap_to_fit_rotated==5 || gap_to_fit_rotated==6|| gap_to_fit_rotated==7 || gap_to_fit_rotated==8){     
        for(unsigned int i=0; i<vec_lepton_bars_rotated.size(); i++){
          if(Yloc[vec_lepton_bars_rotated[i]] > Xloc[vec_lepton_bars_rotated[i]]*TOF1_line_slope[gap_to_fit_rotated-1]+b_pruning_line){
            vec_lepton_TBR_rotated.push_back(vec_lepton_bars_rotated[i]);
          }
      ...
      else if(gap_to_fit_rotated==3){
        for(unsigned int i=0; i<vec_lepton_bars_rotated.size(); i++){
          if(Xloc[vec_lepton_bars_rotated[i]]<vec_xx_kstop_0[0]){
          vec_lepton_TBR_rotated.push_back(vec_lepton_bars_rotated[i]);
          }
        }
      }
      else if(gap_to_fit_rotated==9){
        for(unsigned int i=0; i<vec_lepton_bars_rotated.size(); i++){
          if(Xloc[vec_lepton_bars_rotated[i]]>vec_xx_kstop_0[0]){
            vec_lepton_TBR_rotated.push_back(vec_lepton_bars_rotated[i]);
          }
    ...
    if(vec_kaon_bars.size()>0){
      if(gap_to_fit==1 || gap_to_fit==2 || gap_to_fit==10 || gap_to_fit==11 || gap_to_fit==12){ 
        for(unsigned int i=0; i<vec_lepton_bars.size(); i++){
          if(Yloc[vec_lepton_bars[i]] < Xloc[vec_lepton_bars[i]]*TOF1_line_slope[gap_to_fit-1]+b_pruning_line){
            vec_lepton_TBR.push_back(vec_lepton_bars[i]);
          }
        } 
      }
      else if(gap_to_fit==4 || gap_to_fit==5 || gap_to_fit==6|| gap_to_fit==7 || gap_to_fit==8){     
        for(unsigned int i=0; i<vec_lepton_bars.size(); i++){
          if(Yloc[vec_lepton_bars[i]] > Xloc[vec_lepton_bars[i]]*TOF1_line_slope[gap_to_fit-1]+b_pruning_line){
            vec_lepton_TBR.push_back(vec_lepton_bars[i]);
          }
      ...
      else if(gap_to_fit==3){
        for(unsigned int i=0; i<vec_lepton_bars.size(); i++){
          if(Xloc[vec_lepton_bars[i]]<vec_xx_kstop_0[0]){
            vec_lepton_TBR.push_back(vec_lepton_bars[i]);
          }
        }
      }
      else if(gap_to_fit==9){
        for(unsigned int i=0; i<vec_lepton_bars.size(); i++){
          if(Xloc[vec_lepton_bars[i]]>vec_xx_kstop_0[0]){
            vec_lepton_TBR.push_back(vec_lepton_bars[i]);
          }
  ...
  if(To_rotate_pre_prunning){
    ...
    for(unsigned int i=0; i<vec_lepton_TBR_rotated.size(); i++){
      vec_lepton_bars_rotated.erase(find(vec_lepton_bars_rotated.begin(), vec_lepton_bars_rotated.end(), vec_lepton_TBR_rotated[i]));
    }
    ...
    //else{
      for(unsigned int j=0; j<vec_lepton_bars_rotated.size(); j++){
        if(vec_lepton_bars_rotated[j] != TARGET_Rotated_index[kaon_substitute]){
          vec_lepton_pruned_line_rotated.push_back(vec_lepton_bars_rotated[j]);
      
          vec_xx_lepton_pruned_line_rotated.push_back(Xloc[vec_lepton_bars_rotated[j]]);
          vec_ex_lepton_pruned_line_rotated.push_back(TARGET_Errors_X);

          vec_yy_lepton_pruned_line_rotated.push_back(Yloc[vec_lepton_bars_rotated[j]]);
          vec_ey_lepton_pruned_line_rotated.push_back(TARGET_Errors_Y);   

          vec_xx_lepton_pruned_line_plot.push_back(Xloc[vec_lepton_bars_rotated[j]]);
          vec_yy_lepton_pruned_line_plot.push_back(Yloc[vec_lepton_bars_rotated[j]]);
        }
      }
    //}
    ...
    //  vec_xx_lepton_pruned_line_rotated.push_back(TOF1_Xloc[gap_to_fit_rotated-1][1]);
    //  vec_ex_lepton_pruned_line_rotated.push_back(TOF1_Errors_X[gap_to_fit_rotated-1][1]);
  
    //  vec_yy_lepton_pruned_line_rotated.push_back(TOF1_Yloc[gap_to_fit_rotated-1][1]);
    //  vec_ey_lepton_pruned_line_rotated.push_back(TOF1_Errors_Y[gap_to_fit_rotated-1][1]);
    //}
    
    //graph_lepton_pr2 = new TGraphErrors(vec_xx_lepton_pruned_rotated.size(),&vec_xx_lepton_pruned_rotated[0],&vec_yy_lepton_pruned_rotated[0],
    //                                 &vec_ex_lepton_pruned_rotated[0],&vec_ey_lepton_pruned_rotated[0]);

    graph_lepton_pr2 = new TGraphErrors(vec_xx_lepton_pruned_line_rotated.size(),&vec_xx_lepton_pruned_line_rotated[0],&vec_yy_lepton_pruned_line_rotated[0],
                                     &vec_ex_lepton_pruned_line_rotated[0],&vec_ey_lepton_pruned_line_rotated[0]);
  }
  
  else{
  
    //cout << "SIZE BEFORE : " << vec_lepton_bars.size() << endl;
    for(unsigned int i=0; i<vec_lepton_TBR.size(); i++){
      vec_lepton_bars.erase(find(vec_lepton_bars.begin(), vec_lepton_bars.end(), vec_lepton_TBR[i]));
    }
    //cout << "SIZE AFTER : " << vec_lepton_bars.size() << endl;
  
    for(unsigned int j=0; j<vec_lepton_bars.size(); j++){
      if(vec_lepton_bars[j] != kaon_substitute){
        vec_lepton_pruned_line.push_back(vec_lepton_bars[j]);

        vec_xx_lepton_pruned_line.push_back(Xloc[vec_lepton_bars[j]]);
        vec_ex_lepton_pruned_line.push_back(TARGET_Errors_X);

        vec_yy_lepton_pruned_line.push_back(Yloc[vec_lepton_bars[j]]);
        vec_ey_lepton_pruned_line.push_back(TARGET_Errors_Y);   

        vec_xx_lepton_pruned_line_plot.push_back(Xloc[vec_lepton_bars[j]]);
        vec_yy_lepton_pruned_line_plot.push_back(Yloc[vec_lepton_bars[j]]);
      }
    }

    //for(int k=0; k<weight_TOF1_fit1; k++){
    //  vec_xx_lepton_pruned_line.push_back(TOF1_Xloc[gap_to_fit-1][1]);
    //  vec_ex_lepton_pruned_line.push_back(TOF1_Errors_X[gap_to_fit-1][1]);
  
    //  vec_yy_lepton_pruned_line.push_back(TOF1_Yloc[gap_to_fit-1][1]);
    //  vec_ey_lepton_pruned_line.push_back(TOF1_Errors_Y[gap_to_fit-1][1]);
    //}
  
    //graph_lepton_pr2 = new TGraphErrors(vec_xx_lepton_pruned.size(),&vec_xx_lepton_pruned[0],&vec_yy_lepton_pruned[0],
    //                                   &vec_ex_lepton_pruned[0],&vec_ey_lepton_pruned[0]);

    graph_lepton_pr2 = new TGraphErrors(vec_xx_lepton_pruned_line.size(),&vec_xx_lepton_pruned_line[0],&vec_yy_lepton_pruned_line[0],
                                       &vec_ex_lepton_pruned_line[0],&vec_ey_lepton_pruned_line[0]);
 }

  TGraph *gr_pruning_line_plot = new TGraph(vec_xx_lepton_pruned_line_plot.size(), &vec_xx_lepton_pruned_line_plot[0], &vec_yy_lepton_pruned_line_plot[0]);

  graph_lepton_pr2->SetMarkerColor(2);
  graph_lepton_pr2->SetMarkerStyle(21);
  graph_lepton_pr2->SetMarkerSize(0.8);
  graph_lepton_pr2->GetYaxis()->SetTitle("Y title");
  graph_lepton_pr2->GetXaxis()->SetLimits(-50.,50.);
  graph_lepton_pr2->GetYaxis()->SetRangeUser(-50.,50.);

  gr_pruning_line_plot->SetMarkerColor(3);
  gr_pruning_line_plot->SetMarkerStyle(21);
  gr_pruning_line_plot->SetMarkerSize(0.8);


  if(vec_kaon_bars.size()>0){
    graph_kstop_0->SetMarkerStyle(34);
    graph_kstop_0->SetMarkerColor(1);
    graph_kstop_0->SetMarkerSize(1.3);
  }

  //cout << "TO ROTATE :: " << To_rotate << endl;   

  //if(To_rotate){
  if(To_rotate_pre_prunning){
    for(unsigned int i=0; i<vec_lepton_pruned_line_rotated.size(); i++){
      vec_lepton_bars_pruned_unique.push_back(vec_lepton_pruned_line_rotated[i]);
      //vec_lepton_bars_pruned_rotated.push_back(TARGET_Rotated_index[vec_lepton_bars_pruned[i]]);
    }
  }
  else{
    for(unsigned int i=0; i<vec_lepton_pruned_line.size(); i++){
      vec_lepton_bars_pruned_unique.push_back(vec_lepton_pruned_line[i]);
      //vec_lepton_bars_pruned_rotated.push_back(TARGET_Rotated_index[vec_lepton_bars_pruned[i]]);
    }
  }
 

  sort(vec_lepton_bars_pruned_unique.begin(), vec_lepton_bars_pruned_unique.end() );
  vec_lepton_bars_pruned_unique.erase(unique(vec_lepton_bars_pruned_unique.begin(), vec_lepton_bars_pruned_unique.end()), vec_lepton_bars_pruned_unique.end() );

  //for(unsigned int k=0; k<vec_lepton_bars_pruned_unique.size(); k++){
  //  cout << vec_lepton_bars_pruned_unique[k] << endl;
  //} 
  ...
  double d_angle;
  bool repruned_flag = false;

  //if(To_rotate){
  if(To_rotate_pre_prunning){
    if(gap_to_fit_rotated==1) d_angle = 60. - 90.;
    if(gap_to_fit_rotated==2) d_angle = 30. - 90.;
    if(gap_to_fit_rotated==3) d_angle = 0. - 90.;
    if(gap_to_fit_rotated==4) d_angle = 330. - 90.;
    if(gap_to_fit_rotated==5) d_angle = 300. - 90.;
    if(gap_to_fit_rotated==6) d_angle = 270. - 90.;
    if(gap_to_fit_rotated==7) d_angle = 240. - 90.;
    if(gap_to_fit_rotated==8) d_angle = 210. - 90.;
    if(gap_to_fit_rotated==9) d_angle = 180. - 90.;
    if(gap_to_fit_rotated==10) d_angle = 150. - 90.;
    if(gap_to_fit_rotated==11) d_angle = 120. - 90.;
    if(gap_to_fit_rotated==12) d_angle = 90. - 90.;   
  }
  else{
    if(gap_to_fit==1) d_angle = 60. - 90.;
    if(gap_to_fit==2) d_angle = 30. - 90.;
    if(gap_to_fit==3) d_angle = 0. - 90.;
    if(gap_to_fit==4) d_angle = 330. - 90.;
    if(gap_to_fit==5) d_angle = 300. - 90.;
    if(gap_to_fit==6) d_angle = 270. - 90.;
    if(gap_to_fit==7) d_angle = 240. - 90.;
    if(gap_to_fit==8) d_angle = 210. - 90.;
    if(gap_to_fit==9) d_angle = 180. - 90.;
    if(gap_to_fit==10) d_angle = 150. - 90.;
    if(gap_to_fit==11) d_angle = 120. - 90.;
    if(gap_to_fit==12) d_angle = 90. - 90.;
  }
  ...
  double a_para_pruning = 999.;
  double b_para_pruning = 999.;


  //double d_plus = 6.;   double d_moins = -6.;
  //double dx_plus = 999.;  double dx_moins = 999.;
  //double dy_plus = 999.;  double dy_moins = 999.;

  ddx = dd*sin(d_angle*PI/180);
  ddy = dd*cos(d_angle*PI/180);
  
  //dx_plus = dd*sin(d_angle*PI/180);
  //dy_plus = dd*cos(d_angle*PI/180);
  //dx_moins = dd*sin(d_angle*PI/180);
  //dy_moins = dd*cos(d_angle*PI/180);

  //TLine *para_plus = new TLine(x_pruning_min+dx_plus, y_pruning_min-dy_plus, x_pruning_max+dx_plus, y_pruning_max-dy_plus);
  //TLine *para_moins = new TLine(x_pruning_min+dx_moins, y_pruning_min-dy_moins, x_pruning_max+dx_moins, y_pruning_max-dy_moins);
  TLine *para_pruning = new TLine(x_pruning_min+ddx, y_pruning_min-ddy, x_pruning_max+ddx, y_pruning_max-ddy);

  a_para_pruning = ((y_pruning_min-ddy)-(y_pruning_max-ddy))/((x_pruning_min+ddx)-(x_pruning_max+ddx));
  b_para_pruning = (y_pruning_min-ddy) - a_para_pruning*(x_pruning_min+ddx);

  //para_moins->SetLineWidth(2);
  //para_moins->SetLineColor(6);

  //para_plus->SetLineWidth(2);
  //para_plus->SetLineColor(6);

  para_pruning->SetLineWidth(2);
  para_pruning->SetLineColor(6);

  //cout << "WAHT" << endl;
  //cout << "SIZE : " << vec_lepton_bars_pruned_unique.size() << endl;
  //cout << "CLUSTER SIZE : " << lepton_cluster_size << endl;

  /////////////////////////////////////////////
  /// REPRUNING
  if(vec_lepton_bars_pruned_unique.size()<lepton_cluster_size){
    
    vec_lepton_pruned_line.clear();
    vec_lepton_pruned_line_rotated.clear();
    vec_lepton_TBR_rotated.clear();
    vec_lepton_TBR.clear();

    vec_lepton_bars_rotated.clear();
    vec_xx_lepton_pruned_rotated.clear();
    vec_yy_lepton_pruned_rotated.clear();
    vec_ex_lepton_pruned_rotated.clear();
    vec_ey_lepton_pruned_rotated.clear();
    ...
    if(To_rotate_pre_prunning){
      ...
      if(vec_kaon_bars_rotated.size()>0){
        ...
        if(gap_to_fit_rotated==1 || gap_to_fit_rotated==2 || gap_to_fit_rotated==10 || gap_to_fit_rotated==11 || gap_to_fit_rotated==12){ 
          for(unsigned int i=0; i<vec_lepton_bars_rotated_initial.size(); i++){
            if(Yloc[vec_lepton_bars_rotated_initial[i]] < a_para_pruning*Xloc[vec_lepton_bars_rotated_initial[i]]+b_para_pruning){
              vec_lepton_TBR_rotated.push_back(vec_lepton_bars_rotated_initial[i]);
            }
          } 
        }    
        else if(gap_to_fit_rotated==4 || gap_to_fit_rotated==5 || gap_to_fit_rotated==6|| gap_to_fit_rotated==7 || gap_to_fit_rotated==8){     
          ...
          for(unsigned int i=0; i<vec_lepton_bars_rotated_initial.size(); i++){
            if(Xloc[vec_lepton_bars_rotated_initial[i]]<vec_xx_kstop_0[0]+ddx){
              vec_lepton_TBR_rotated.push_back(vec_lepton_bars_rotated_initial[i]);
            }
          }
        }
      ...
      if(vec_kaon_bars.size()>0){
        if(gap_to_fit==1 || gap_to_fit==2 || gap_to_fit==10 || gap_to_fit==11 || gap_to_fit==12){ 
          for(unsigned int i=0; i<vec_lepton_bars_initial.size(); i++){
            if(Yloc[vec_lepton_bars_initial[i]] < a_para_pruning*Xloc[vec_lepton_bars_initial[i]]+b_para_pruning){
              vec_lepton_TBR.push_back(vec_lepton_bars_initial[i]);
            }
          } 
        }
        else if(gap_to_fit==4 || gap_to_fit==5 || gap_to_fit==6|| gap_to_fit==7 || gap_to_fit==8){     
          ...
          for(unsigned int i=0; i<vec_lepton_bars_initial.size(); i++){
            if(Xloc[vec_lepton_bars_initial[i]]<vec_xx_kstop_0[0]+ddx){
              vec_lepton_TBR.push_back(vec_lepton_bars_initial[i]);
            }
          }
        }
    ...
    vec_xx_lepton_pruned_line_plot.clear();
    vec_yy_lepton_pruned_line_plot.clear();

    //if(To_rotate){
    if(To_rotate_pre_prunning){
     for(unsigned int i=0; i<vec_lepton_TBR_rotated.size(); i++){
        vec_lepton_bars_rotated_initial.erase(find(vec_lepton_bars_rotated_initial.begin(), vec_lepton_bars_rotated_initial.end(), vec_lepton_TBR_rotated[i]));
      }

      for(unsigned int j=0; j<vec_lepton_bars_rotated_initial.size(); j++){
         if(vec_lepton_bars_rotated_initial[j] != TARGET_Rotated_index[kaon_substitute]){
          vec_lepton_pruned_line_rotated.push_back(vec_lepton_bars_rotated_initial[j]);

          vec_xx_lepton_pruned_line_rotated.push_back(Xloc[vec_lepton_bars_rotated_initial[j]]);
          vec_ex_lepton_pruned_line_rotated.push_back(TARGET_Errors_X);

          vec_yy_lepton_pruned_line_rotated.push_back(Yloc[vec_lepton_bars_rotated_initial[j]]);
          vec_ey_lepton_pruned_line_rotated.push_back(TARGET_Errors_Y);   

          vec_xx_lepton_pruned_line_plot.push_back(Xloc[vec_lepton_bars_rotated_initial[j]]);
          vec_yy_lepton_pruned_line_plot.push_back(Yloc[vec_lepton_bars_rotated_initial[j]]);
        }
      }
      ...
      for(unsigned int i=0; i<vec_lepton_TBR.size(); i++){
        vec_lepton_bars_initial.erase(find(vec_lepton_bars_initial.begin(), vec_lepton_bars_initial.end(), vec_lepton_TBR[i]));
      }

      for(unsigned int j=0; j<vec_lepton_bars_initial.size(); j++){
        if(vec_lepton_bars_initial[j] != kaon_substitute){
          vec_lepton_pruned_line.push_back(vec_lepton_bars_initial[j]);

          vec_xx_lepton_pruned_line.push_back(Xloc[vec_lepton_bars_initial[j]]);
          vec_ex_lepton_pruned_line.push_back(TARGET_Errors_X);

          vec_yy_lepton_pruned_line.push_back(Yloc[vec_lepton_bars_initial[j]]);
          vec_ey_lepton_pruned_line.push_back(TARGET_Errors_Y);   
        
          vec_xx_lepton_pruned_line_plot.push_back(Xloc[vec_lepton_bars_initial[j]]);
          vec_yy_lepton_pruned_line_plot.push_back(Yloc[vec_lepton_bars_initial[j]]);
        }
      }
    ...
    //cout << "ALBANE TEST : " << vec_xx_lepton_pruned_line_plot.size() << endl;

    gr_pruning_line_plot = new TGraph(vec_xx_lepton_pruned_line_plot.size(),
                                      &vec_xx_lepton_pruned_line_plot[0],
                                      &vec_yy_lepton_pruned_line_plot[0]);
  ...
  if(To_rotate_pre_prunning){
    ...
    //for(unsigned int i=0; i<vec_lepton_bars_rotated_initial.size(); i++){
    for(unsigned int i=0; i<vec_lepton_pruned_line_rotated.size(); i++){
      if(vec_lepton_pruned_line_rotated[i] != kaon_substitute){
        //vec_lepton_bars_pruned.push_back(TARGET_Rotated_index_inverse[vec_lepton_bars_rotated[i]]);
        vec_lepton_pruned_line.push_back(TARGET_Rotated_index_inverse[vec_lepton_pruned_line_rotated[i]]);

        //vec_xx_lepton_pruned.push_back(Xloc[TARGET_Rotated_index_inverse[vec_lepton_bars_rotated[i]]]);
        //vec_ex_lepton_pruned.push_back(TARGET_Errors_X);
        vec_xx_lepton_pruned_line.push_back(Xloc[TARGET_Rotated_index_inverse[vec_lepton_pruned_line_rotated[i]]]);
        vec_ex_lepton_pruned_line.push_back(TARGET_Errors_X);

        //vec_yy_lepton_pruned.push_back(Yloc[TARGET_Rotated_index_inverse[vec_lepton_bars_rotated[i]]]);
        //vec_ey_lepton_pruned.push_back(TARGET_Errors_Y);
        vec_yy_lepton_pruned_line.push_back(Yloc[TARGET_Rotated_index_inverse[vec_lepton_pruned_line_rotated[i]]]);
        vec_ey_lepton_pruned_line.push_back(TARGET_Errors_Y);
      }
    }
  ...
  TGraph *graph_lepton_pr3;
  graph_lepton_pr3 = new TGraphErrors(vec_xx_lepton_pruned_line.size(),&vec_xx_lepton_pruned_line[0],&vec_yy_lepton_pruned_line[0],
                                     &vec_ex_lepton_pruned_line[0],&vec_ey_lepton_pruned_line[0]);
  
  TGraph *graph_kaon_pr3;
  graph_kaon_pr3 = new TGraphErrors(vec_xx_kaon.size(),&vec_xx_kaon[0],&vec_yy_kaon[0],
                                     &vec_ex_kaon[0],&vec_ey_kaon[0]);

  graph_lepton_pr3->SetMarkerColor(2);
  graph_lepton_pr3->SetMarkerStyle(21);
  graph_lepton_pr3->SetMarkerSize(0.8);
  graph_lepton_pr3->GetYaxis()->SetTitle("Y title");
  graph_lepton_pr3->GetXaxis()->SetLimits(-50.,50.);
  graph_lepton_pr3->GetYaxis()->SetRangeUser(-50.,50.);

  graph_kaon_pr3->SetMarkerColor(4);
  graph_kaon_pr3->SetMarkerStyle(21);
  graph_kaon_pr3->SetMarkerSize(0.8);
  graph_kaon_pr3->GetYaxis()->SetTitle("Y title");
  graph_kaon_pr3->GetXaxis()->SetLimits(-50.,50.);
  graph_kaon_pr3->GetYaxis()->SetRangeUser(-50.,50.);


  //vector<int> vec_lepton_bars_pruned_rotated;     vec_lepton_bars_pruned_rotated.clear();

  //for(unsigned int i=0; i<vec_lepton_bars_pruned.size(); i++){
  //  vec_lepton_bars_pruned_unique.push_back(vec_lepton_bars_pruned[i]);
  //  vec_lepton_bars_pruned_rotated.push_back(TARGET_Rotated_index[vec_lepton_bars_pruned[i]]);
  //}
  ...
  TH2F *h_Pruning[2];   char Title_Pruning[2][100];   char Name_Pruning[2][100];

  sprintf(Title_Pruning[0],"Linear Pruning  |  Run %d, Evt %d", Run_Number, ievt); 
  sprintf(Name_Pruning[0],"Linear Pruning"); 

  sprintf(Title_Pruning[1],"Triangular Pruning  |  Run %d, Evt %d", Run_Number, ievt); 
  sprintf(Name_Pruning[1],"Triangular Pruning"); 

  h_Pruning[0] = new TH2F(Name_Pruning[0],Title_Pruning[0],100,-50,50,100,-50,50);
  h_Pruning[1] = new TH2F(Name_Pruning[1],Title_Pruning[1],100,-50,50,100,-50,50);

  TGraph *graph_kaon_centroid_0;
  graph_kaon_centroid_0 = new TGraph(vec_xx_kaon_centroid.size(),&vec_xx_kaon_centroid[0],&vec_yy_kaon_centroid[0]);

  graph_kaon_centroid_0->SetMarkerStyle(5);
  graph_kaon_centroid_0->SetMarkerColor(6);
  graph_kaon_centroid_0->SetMarkerSize(2);
  ...
  TF1 *gr_kaon_fit;
  TF1 *func_kaon_fit;
  TF1 *func_kaon_fit_rotate;

  TGraph *gr_lepton_pr;
  TGraph *gr_lepton_pr_rotate;
  TGraph *gr_lepton_1;
  TGraph *gr_lepton_2;
  TGraph *gr_lepton_3;
  TGraph *gr3_Leptons_rotate;
  
  TGraph *graph_kaon_rotate;

  graph_kaon = new TGraphErrors(vec_xx_kaon.size(),&vec_xx_kaon[0],&vec_yy_kaon[0],
                                     &vec_ey_kaon[0],&vec_ey_kaon[0]);

  gr_kaon_bk = new TGraph(vec_xx_kaon.size(),&vec_xx_kaon[0],&vec_yy_kaon[0]);
  
  gr_kaon_fit = new TF1("kaon_fit", "pol1");
  func_kaon_fit = new TF1("g_kaon", "pol1");
  func_kaon_fit_rotate = new TF1("gr_kaon_rotate","pol1");

  double a_kaon_fit_rotate=999.;
  double b_kaon_fit_rotate=999.;
  double ChiS_kaon_rotate;
  double ndf_kaon_rotate;
  ...
  bool TO_ROTATE_KAON = false;
  bool to_draw = false;

  graph_kaon->SetMarkerColor(4);
  graph_kaon->SetMarkerStyle(21);
  graph_kaon->SetMarkerSize(0.8);
  graph_kaon->GetYaxis()->SetTitle("Y title");
  graph_kaon->GetXaxis()->SetLimits(-50.,50.);
  graph_kaon->GetYaxis()->SetRangeUser(-50.,50.);


  if(vec_xx_kaon.size()>kaon_cluster_size){ 
    vertical_kaon = check_vertical_kaon(vec_xx_kaon);
    if(!vertical_kaon) {
      graph_kaon->Fit("g_kaon","QW");
      graph_kaon->Fit("g_kaon","Q");
      func_kaon_fit = graph_kaon->GetFunction("g_kaon");
      func_kaon_fit->SetLineWidth(2);
      func_kaon_fit->SetLineColor(4);
      a_fit_kaon = func_kaon_fit->GetParameter(1);
      b_fit_kaon = func_kaon_fit->GetParameter(0);
      Chis_kaon = func_kaon_fit->GetChisquare();
      ndf_kaon = func_kaon_fit->GetNDF();
    }

    if(abs(a_fit_kaon)>20) TO_ROTATE_KAON = true;
  }

  //cout << "TEST : " << TO_ROTATE_KAON << endl;
  //if(abs(a_fit_kaon)>20) TO_ROTATE_KAON = true;
  //cout << "TEST : " << TO_ROTATE_KAON << endl;

  if(vec_xx_kaon.size()>=kaon_cluster_size){
    if(TO_ROTATE_KAON){
      for(unsigned int i=0; i<vec_xx_kaon.size(); i++){
        vec_kaon_xx_rotate.push_back(Xloc[TARGET_Rotated_index[vec_kaon_bars[i]]]);
        vec_kaon_yy_rotate.push_back(Yloc[TARGET_Rotated_index[vec_kaon_bars[i]]]);
        vec_kaon_ex_rotate.push_back(TARGET_Errors_X);
        vec_kaon_ey_rotate.push_back(TARGET_Errors_Y);
      }
    }
  }

  if(TO_ROTATE_KAON){
    graph_kaon_rotate = new TGraphErrors(vec_kaon_xx_rotate.size(),&vec_kaon_xx_rotate[0],&vec_kaon_yy_rotate[0],
       &vec_kaon_ex_rotate[0],&vec_kaon_ey_rotate[0]);
    graph_kaon_rotate->SetMarkerColor(4);
    graph_kaon_rotate->SetMarkerStyle(33);
    graph_kaon_rotate->SetMarkerSize(1.5);
    graph_kaon_rotate->GetYaxis()->SetTitle("Y title");
    graph_kaon_rotate->GetXaxis()->SetLimits(-50.,50.);
    graph_kaon_rotate->GetYaxis()->SetRangeUser(-50.,50.);
  }
  else{
    graph_kaon_rotate = new TGraphErrors(vec_xx_kaon.size(),&vec_xx_kaon[0],&vec_yy_kaon[0],
       &vec_ex_kaon[0],&vec_ey_kaon[0]);  
    graph_kaon_rotate->SetMarkerColor(4);
    graph_kaon_rotate->SetMarkerStyle(21);
    graph_kaon_rotate->SetMarkerSize(0.8);
    graph_kaon_rotate->GetYaxis()->SetTitle("Y title");
    graph_kaon_rotate->GetXaxis()->SetLimits(-50.,50.);
    graph_kaon_rotate->GetYaxis()->SetRangeUser(-50.,50.);
  }

  double A_FIT_KAON = 999.;
  double B_FIT_KAON = 999.;


  if(vec_xx_kaon.size()>kaon_cluster_size){
    if(TO_ROTATE_KAON){
      graph_kaon_rotate->Fit("gr_kaon_rotate","QW");
      graph_kaon_rotate->Fit("gr_kaon_rotate","Q");
      func_kaon_fit_rotate = graph_kaon_rotate->GetFunction("gr_kaon_rotate");
      func_kaon_fit_rotate->SetLineWidth(2);
      func_kaon_fit_rotate->SetLineColor(4);
      a_kaon_fit_rotate = func_kaon_fit_rotate->GetParameter(1);
      b_kaon_fit_rotate = func_kaon_fit_rotate->GetParameter(0);
      ChiS_kaon_rotate = func_kaon_fit_rotate->GetChisquare();
      ndf_kaon_rotate = func_kaon_fit_rotate->GetNDF();
      A_FIT_KAON = -1/a_kaon_fit_rotate;
      B_FIT_KAON = b_kaon_fit_rotate/a_kaon_fit_rotate;
    }
    else{
      graph_kaon_rotate->Fit("gr_kaon_rotate","Q");
      func_kaon_fit_rotate = graph_kaon_rotate->GetFunction("gr_kaon_rotate");
  ...
  if(TO_ROTATE_KAON) to_draw = true;
  if(TO_ROTATE_KAON && abs(a_kaon_fit_rotate)>100) to_draw = false;

  TLine *best_kaon_fit_rotate;
  if(TO_ROTATE_KAON){
    best_kaon_fit_rotate = new TLine(50*a_kaon_fit_rotate + b_kaon_fit_rotate,-50,-50*a_kaon_fit_rotate + b_kaon_fit_rotate,50); //ROTATE_CHANGE
    best_kaon_fit_rotate->SetLineWidth(2);
    best_kaon_fit_rotate->SetLineColor(4);
  }
  else{
    best_kaon_fit_rotate = new TLine(-50, a_fit_kaon*(-50)+b_fit_kaon,50,a_fit_kaon*(-50)+b_fit_kaon); //ROTATE_CHANGE
   }
...
*/
  ...
  vec_lepton_bars_pruned.clear();

  //cout << vec_lepton_pruned_polygon.size() << endl;

  //cout << endl;
  //cout << "POLYGON or LINE " << endl;
  //cout << "SIZE : " << vec_lepton_pruned_line.size() << endl;
  //for(unsigned int k=0; k<vec_lepton_pruned_line.size(); k++){
  //  cerr << k << "   " << vec_lepton_pruned_line[k] << endl;
  //}
  //cout << endl;
  ...
  if(flag_pruning_line){
    ...
    if(To_rotate_pre_prunning){
      for(unsigned int i=0; i<vec_lepton_pruned_line.size(); i++){
        vec_lepton_bars_pruned.push_back(TARGET_Rotated_index[vec_lepton_pruned_line[i]]);
      }
    }
    else{
      for(unsigned int i=0; i<vec_lepton_pruned_line.size(); i++){
        vec_lepton_bars_pruned.push_back(vec_lepton_pruned_line[i]); 
      }   
    }
  }
  else{
    for(unsigned int i=0; i<vec_lepton_pruned_polygon.size(); i++){
      //cerr << i << "  " << vec_lepton_pruned_polygon[i] << endl;
      vec_lepton_bars_pruned.push_back(vec_lepton_pruned_polygon[i]);
    }
  }
  ...
  for(unsigned int i=0; i<vec_lepton_bars_pruned.size(); i++){
    ...
    vec_xx_lepton.push_back(Xloc[vec_lepton_bars_pruned[i]]);
    vec_ex_lepton.push_back(TARGET_Errors_X);

    vec_yy_lepton.push_back(Yloc[vec_lepton_bars_pruned[i]]);
    vec_ey_lepton.push_back(TARGET_Errors_Y);
  }
  ...
  double a_lepton_fit_pr = 0.;
  double b_lepton_fit_pr = 0.;
  double Chis_lepton_fit_pr = 0.;
  double ndf_lepton_fit_pr = 0;

  gr_lepton_pr = new TGraphErrors(vec_xx_lepton.size(),&vec_xx_lepton[0],&vec_yy_lepton[0],
    &vec_ex_lepton[0],&vec_ey_lepton[0]);
  
  func_lepton_fit_0 = new TF1("Lepton_fit_pr", "pol1");
  
  gr_lepton_pr->GetXaxis()->SetLimits(-50.,50.);
  gr_lepton_pr->GetYaxis()->SetRangeUser(-50.,50.);


  if(gap_to_fit==1 || gap_to_fit==2 || gap_to_fit==7 || gap_to_fit==8){
    func_lepton_fit_0->SetParameter(0,0);
    func_lepton_fit_0->SetParameter(1,1);
  }
  ...
  //gr_lepton_pr->Fit("Lepton_fit_pr","QW");
  gr_lepton_pr->Fit("Lepton_fit_pr","Q");

  //delete func_lepton_fit_1;
  TF1 *func_lepton_fit_prr = gr_lepton_pr->GetFunction("Lepton_fit_pr");
  a_lepton_fit_pr = func_lepton_fit_prr->GetParameter(1);
  b_lepton_fit_pr = func_lepton_fit_prr->GetParameter(0);
  Chis_lepton_fit_pr = func_lepton_fit_prr->GetChisquare();
  ndf_lepton_fit_pr = func_lepton_fit_prr->GetNDF();
  func_lepton_fit_prr->SetLineWidth(2);
  func_lepton_fit_prr->SetLineColor(2);

  bool To_rotate_post_prunning;
  To_rotate_post_prunning = false;

  //if(abs(a_lepton_fit_pr) > 15) To_rotate = true;
  if(abs(a_lepton_fit_pr) > 15) To_rotate_post_prunning = true;

 
  //if(To_rotate){
  if(To_rotate_post_prunning){

    gap_to_fit_rotated = TOF1_rotated[gap_to_fit-1]+1;

    for(unsigned int i=0; i<vec_lepton_bars_pruned.size(); i++){
    //for(unsigned int i=0; i<vec_lepton_bars.size(); i++){
      vec_xx_lepton_rotate.push_back(Xloc[TARGET_Rotated_index[vec_lepton_bars_pruned[i]]]);
      vec_ex_lepton_rotate.push_back(TARGET_Errors_Y);
      vec_yy_lepton_rotate.push_back(Yloc[TARGET_Rotated_index[vec_lepton_bars_pruned[i]]]);
      vec_ey_lepton_rotate.push_back(TARGET_Errors_Y);
    }

    for(int i = 0; i<weight_TOF1_fit1; i++){
      vec_xx_lepton_rotate.push_back(Gap[gap_to_fit_rotated-1][1][0]);
      vec_ex_lepton_rotate.push_back(TOF1_Errors_X[gap_to_fit_rotated-1][1]);
      vec_yy_lepton_rotate.push_back(Gap[gap_to_fit_rotated-1][1][1]);
      vec_ey_lepton_rotate.push_back(TOF1_Errors_Y[gap_to_fit_rotated-1][1]);
    }
    ...
    gr_lepton_pr = new TGraphErrors(vec_xx_lepton_rotate.size(),&vec_xx_lepton_rotate[0],&vec_yy_lepton_rotate[0],
    &vec_ex_lepton_rotate[0],&vec_ey_lepton_rotate[0]);
  
    gr_lepton_pr->GetXaxis()->SetLimits(-50.,50.);
    gr_lepton_pr->GetYaxis()->SetRangeUser(-50.,50.);

    TF1 *func_lepton_fit_0 = new TF1("Lepton_fit_pr", "pol1");

    if(gap_to_fit_rotated==1 || gap_to_fit_rotated==2 || gap_to_fit_rotated==7 || gap_to_fit_rotated==8){
      func_lepton_fit_0->SetParameter(0,0);
      func_lepton_fit_0->SetParameter(1,1);
    }
    if(gap_to_fit_rotated==4 || gap_to_fit_rotated==5 || gap_to_fit_rotated==10 || gap_to_fit_rotated==11){
      func_lepton_fit_0->SetParameter(0,0);
      func_lepton_fit_0->SetParameter(1,-1);
    }
  ...
  TLine *L_TEST = new TLine(-50., -21.6279, 50., 36.1071);


  /// Kstop calculation

  vector<double> vec_intersect_prr;      vec_intersect_prr.clear();

  vec_intersect_prr = _2lines_intersect(a_fit_kaon, b_fit_kaon, a_lepton_fit_pr, b_lepton_fit_pr);


  ///////////////////////  K_STOP DETERMINATION  /////////////////////
  ...
  if(vec_kaon_bars.size()>=kaon_cluster_size){
    if(vertical_kaon){
      //if(To_rotate){
      if(To_rotate_pre_prunning || To_rotate_post_prunning){
        //cout << "ALBANE" << endl;
        ...
        //cout << "ALBANE 2" << endl;
        vec_fit_lines_intersect.push_back(vec_xx_kaon[0]);
        vec_fit_lines_intersect.push_back(a_lepton_fit_pr * vec_xx_kaon[0] + b_lepton_fit_pr);
      }
    }
    else {
      //if(To_rotate){
      if(To_rotate_pre_prunning || To_rotate_post_prunning){
        if(TO_ROTATE_KAON){
          vec_fit_lines_intersect = _2lines_intersect(a_fit_lepton_pr, b_fit_lepton_pr, A_FIT_KAON, B_FIT_KAON);
        }
        else{
          //cout << "IM HERE" << endl;
        ...
        if(TO_ROTATE_KAON){
          if(abs(a_kaon_fit_rotate)>100){
            vec_fit_lines_intersect.push_back(999.99);
          }
          //else vec_fit_lines_intersect = _2lines_intersect(a_lepton_fit_pr, b_lepton_fit_pr, A_FIT_KAON, B_FIT_KAON);
          else vec_fit_lines_intersect = _2lines_intersect(a_fit_lepton_pr, b_fit_lepton_pr, A_FIT_KAON, B_FIT_KAON);
        }
        else{
          vec_fit_lines_intersect = _2lines_intersect(a_fit_lepton_pr, b_fit_lepton_pr, a_fit_kaon, b_fit_kaon);
        }
  ...
  else if(TO_ROTATE_KAON && abs(a_kaon_fit_rotate)>100){
    vec_k_stop_coordinates.push_back(vec_kaon_centroid_coordinates[0]);
    vec_k_stop_coordinates.push_back(vec_kaon_centroid_coordinates[1]);   
  }
  else{
  ...
  dist = distance(TOF1_Errors_X[gap_to_fit-1][1],TOF1_Errors_Y[gap_to_fit-1][1],vec_intersect_prr[0],vec_intersect_prr[1]);
  ...
  perp_distance = sqrt(pow((TOF1_Xloc[gap_to_fit-1][2]-vec_intersect_prr[0]),2) + 
    pow((TOF1_Yloc[gap_to_fit-1][1]-vec_intersect_prr[1]),2));

  double X_pink, Y_pink;
  X_pink = TOF1_Xloc[gap_to_fit-1][2] + (perp_distance-6)*cos(60*PI/180);
  Y_pink = TOF1_Yloc[gap_to_fit-1][2] - (perp_distance-6)*sin(60*PI/180);

  double b_pink;
  b_pink = Y_pink - TOF1_line_slope[gap_to_fit-1]*X_pink;
  ...
  //cout << endl;
  //cout << "SIZE : " << vec_xx_lepton.size() << endl;
  //for(unsigned int j=0; j<vec_xx_lepton.size(); j++){
  //  cerr << j << "  " << vec_xx_lepton[j] << "   " << vec_yy_lepton[j] << endl;
  //}
  //cout << endl;
  ...
  //for(unsigned int i=0; i<vec_xx_lepton.size(); i++){
  //  cerr << vec_xx_lepton[i] << "  ";
  //}
  //cout << endl;

  //cout << "BEFORE 2222 : " << vec_xx_lepton.size() << endl;

 if(vec_lepton_bars_pruned.size()>=lepton_cluster_size){
 ...
 //if(abs(a_lepton_fit_1)>15) To_rotate = true;
  vector<int> temp; temp.clear();

  if(To_rotate_post_prunning){
    temp = vec_lepton_bars_pruned;
    vec_lepton_bars_pruned.clear();
    for(unsigned int i=0; i<temp.size(); i++){
      vec_lepton_bars_pruned.push_back(TARGET_Rotated_index[temp[i]]);
    }
    temp.clear();
  }
  ...
  if(To_rotate_pre_prunning || To_rotate_post_prunning){
    ...
    //if(To_rotate_pre_prunning){
      for(unsigned int i=0; i<vec_lepton_bars_pruned.size(); i++){
        //vec_xx_lepton.push_back(Xloc[TARGET_Rotated_index[vec_lepton_bars_pruned[i]]]);
        vec_xx_lepton.push_back(Xloc[vec_lepton_bars_pruned[i]]);
        vec_ex_lepton.push_back(TARGET_Errors_X);

        //vec_yy_lepton.push_back(Yloc[TARGET_Rotated_index[vec_lepton_bars_pruned[i]]]);
        vec_yy_lepton.push_back(Yloc[vec_lepton_bars_pruned[i]]);
        vec_ey_lepton.push_back(TARGET_Errors_Y);
      }

      for(int i = 0; i<weight_TOF1_fit1; i++){
        vec_xx_lepton.push_back(Gap[gap_to_fit_rotated-1][1][0]);
        vec_ex_lepton.push_back(TOF1_Errors_X[gap_to_fit_rotated-1][1]);
        vec_yy_lepton.push_back(Gap[gap_to_fit_rotated-1][1][1]);
        vec_ey_lepton.push_back(TOF1_Errors_Y[gap_to_fit_rotated-1][1]);
      }
    //}
    //else if(To_rotate_post_prunning){
/*      for(unsigned int i=0; i<vec_lepton_bars_pruned.size(); i++){
        vec_xx_lepton.push_back(Xloc[TARGET_Rotated_index[vec_lepton_bars_pruned[i]]]);
        //vec_xx_lepton.push_back(Xloc[vec_lepton_bars_pruned[i]]);
        vec_ex_lepton.push_back(TARGET_Errors_X);
...
*/ 
    ...
    //gr_lepton_1 = new TGraphErrors(vec_lepton_bars_pruned_rotated.size(),&vec_xx_lepton_pruned_rotated[0],&vec_yy_lepton_pruned_rotated[0],
    //                                 &vec_ex_lepton_pruned_rotated[0],&vec_ey_lepton_pruned_rotated[0]);
    ...
    TF1 *func_lepton_fit_1_rot = gr_lepton_1->GetFunction("Lepton_fit_1");
    a_lepton_fit_1 = func_lepton_fit_1_rot->GetParameter(1);
    b_lepton_fit_1 = func_lepton_fit_1_rot->GetParameter(0);
    Chis_lepton_fit_1 = func_lepton_fit_1_rot->GetChisquare();
    ndf_lepton_fit_1 = func_lepton_fit_1_rot->GetNDF();
    func_lepton_fit_1_rot->SetLineWidth(2);
    func_lepton_fit_1_rot->SetLineColor(2);
  } 
  ...
  //if(To_rotate){    /// <---- TO REVIEW !
  if(To_rotate_pre_prunning || To_rotate_post_prunning){    /// <---- TO REVIEW 

    for(unsigned int i=0; i<vec_lepton_bars_pruned.size(); i++){
      //if(distance_to_line(Xloc[vec_lepton_bars_pruned[i]],Yloc[vec_lepton_bars_pruned[i]],a_lepton_fit_1,b_lepton_fit_1) <= max_dist
        //&& TARGET_High_has_neighbours[TARGET_Rotated_index_inverse[vec_lepton_bars_pruned[i]]] && !k_stop_bar[TARGET_Rotated_index_inverse[vec_lepton_bars_pruned[i]]]){
        //if(ADC_High_TARGET[TARGET_Rotated_index_inverse[vec_lepton_bars_pruned[i]]]>Angle_ADC_cut && has_TDC_hit[TARGET_Rotated_index_inverse[vec_lepton_bars_pruned[i]]]){
      ...
      if(distance_to_line(Xloc[vec_lepton_bars_pruned[i]],Yloc[vec_lepton_bars_pruned[i]],a_lepton_fit_1,b_lepton_fit_1) <= max_dist
        ...
        if(ADC_High_TARGET[TARGET_Rotated_index_inverse[vec_lepton_bars_pruned[i]]]>Angle_ADC_cut && has_TDC_hit[TARGET_Rotated_index_inverse[vec_lepton_bars_pruned[i]]]){
          ...
          vec_lepton_bars_rotate.push_back(TARGET_Rotated_index_inverse[vec_lepton_bars_pruned[i]]);
          vec_lepton_bars_final.push_back(TARGET_Rotated_index_inverse[vec_lepton_bars_pruned[i]]);
          lepton_counter++;

          if(IsIn(vec_lepton_bars_pruned[i], channel[TOF1_rotated[gap_to_fit-1]][0], channel[TOF1_rotated[gap_to_fit-1]][1],
                  ...
                  channel[TOF1_rotated[gap_to_fit-1]][8], channel[TOF1_rotated[gap_to_fit-1]][9],
                  channel[TOF1_rotated[gap_to_fit-1]][10], channel[TOF1_rotated[gap_to_fit-1]][11])){
        ...
        if(ADC_High_TARGET[TARGET_Rotated_index_inverse[vec_lepton_bars_pruned[i]]]<0 && 
          ADC_Low_TARGET[TARGET_Rotated_index_inverse[vec_lepton_bars_pruned[i]]]>0 && Switch==1){
...
/*  else if(To_rotate_post_prunning){    /// <---- TO REVIEW 
    ...
    for(unsigned int i=0; i<vec_lepton_bars_pruned.size(); i++){
      ...
      if(distance_to_line(Xloc[TARGET_Rotated_index[vec_lepton_bars_pruned[i]]],Yloc[TARGET_Rotated_index[vec_lepton_bars_pruned[i]]],a_lepton_fit_1,b_lepton_fit_1) <= max_dist
        && TARGET_High_has_neighbours[vec_lepton_bars_pruned[i]] && !k_stop_bar[vec_lepton_bars_pruned[i]]){
        if(ADC_High_TARGET[vec_lepton_bars_pruned[i]]>Angle_ADC_cut && has_TDC_hit[vec_lepton_bars_pruned[i]]){
          ...
          vec_lepton_bars_rotate.push_back(vec_lepton_bars_pruned[i]);
          vec_lepton_bars_final.push_back(vec_lepton_bars_pruned[i]);
          lepton_counter++;

          if(IsIn(TARGET_Rotated_index[vec_lepton_bars_pruned[i]], channel[TOF1_rotated[gap_to_fit-1]][0], channel[TOF1_rotated[gap_to_fit-1]][1],
                  channel[TOF1_rotated[gap_to_fit-1]][2], channel[TOF1_rotated[gap_to_fit-1]][3],
                  channel[TOF1_rotated[gap_to_fit-1]][4], channel[TOF1_rotated[gap_to_fit-1]][5],
                  channel[TOF1_rotated[gap_to_fit-1]][6], channel[TOF1_rotated[gap_to_fit-1]][7],
                  channel[TOF1_rotated[gap_to_fit-1]][8], channel[TOF1_rotated[gap_to_fit-1]][9],
        ...
        if(ADC_High_TARGET[vec_lepton_bars_pruned[i]]<0 && 
          ADC_Low_TARGET[vec_lepton_bars_pruned[i]]>0 && Switch==1){
...
*/  else{
    ...
    for(unsigned int i=0; i<vec_lepton_bars_pruned.size(); i++){
      ...
      if(distance_to_line(Xloc[vec_lepton_bars_pruned[i]],Yloc[vec_lepton_bars_pruned[i]],a_lepton_fit_1,b_lepton_fit_1) <= max_dist
        ...
        if(ADC_High_TARGET[vec_lepton_bars_pruned[i]]>Angle_ADC_cut && has_TDC_hit[vec_lepton_bars_pruned[i]]){
          ...
          if(IsIn(vec_lepton_bars_pruned[i],channel[gap_to_fit-1][0], channel[gap_to_fit-1][1],
                    channel[gap_to_fit-1][2], channel[gap_to_fit-1][3],
                    channel[gap_to_fit-1][4], channel[gap_to_fit-1][5],
                    channel[gap_to_fit-1][6], channel[gap_to_fit-1][7],
                    channel[gap_to_fit-1][8], channel[gap_to_fit-1][9],
        ...
        if(ADC_High_TARGET[vec_lepton_bars_pruned[i]]<0 && ADC_Low_TARGET[vec_lepton_bars_pruned[i]]>0 && Switch==1){
          vec_xx_lepton.push_back(Xloc[vec_lepton_bars_pruned[i]]);
          vec_ex_lepton.push_back(TARGET_Errors_X);
  ...
  //cout << vec_lepton_bars.size() << endl;
  //cout << vec_lepton_bars_norepeat.size() << endl;
  ...
  //cout << "BEFORE 3333 : " << vec_xx_lepton.size() << endl;
...
/*  else{
    //cout << "3333" << "   " << vec_xx_lepton.size() << endl;
    if(batch==1){
...
*/
  for(int i=0; i<2; i++){
    ...
    x_int_TOF1[i] = 999.;                          y_int_TOF1[i] = 999.;
  }
  ...
  a_lepton_fit_3 = 999.;
  b_lepton_fit_3 = 999.;
  Chis_lepton_fit_3 = 999.;
  ndf_lepton_fit_3 = 999.;
  ...
  double a_lepton_fit_3_error = 99.;
  double b_lepton_fit_3_error = 99.;
  TF1 *func_lepton_fit_3_ptr = new TF1("lepton_fit_3_ptr", "pol1");
  bool empty_lepton3 = false;


  //cout << "4444 : " << vec_xx_lepton.size() << endl;


  if(vec_xx_lepton.size() <= unsigned(weight_TOF1_fit3)) empty_lepton3=true;

  if((vec_xx_lepton.size()-unsigned(weight_TOF1_fit3))>=lepton_cluster_size){
    //gr_lepton_3->Fit("lepton_fit_3","QW");
    gr_lepton_3->Fit("lepton_fit_3","Q");
    func_lepton_fit_3_ptr = gr_lepton_3->GetFunction("lepton_fit_3");

    a_lepton_fit_3 = func_lepton_fit_3_ptr->GetParameter(1);
    b_lepton_fit_3 = func_lepton_fit_3_ptr->GetParameter(0);
    a_lepton_fit_3_error = func_lepton_fit_3_ptr->GetParError(1);
    b_lepton_fit_3_error = func_lepton_fit_3_ptr->GetParError(0);
  ...
  if(To_rotate_pre_prunning || To_rotate_post_prunning){  // TO MOVE
    for(unsigned int j=0; j<vec_lepton_bars_rotate.size(); j++){
      if(vec_lepton_bars_rotate[j] != kaon_substitute){
  ...
  vector<double> inter_tof1;  inter_tof1.clear();
  //if(To_rotate){
  if(To_rotate_pre_prunning || To_rotate_post_prunning){
    inter_tof1 = _intersect_TOF1(gap_to_fit_rotated-1, a_lepton_fit_3, b_lepton_fit_3);
  }
  else{
     inter_tof1 = _intersect_TOF1(gap_to_fit-1, a_lepton_fit_3, b_lepton_fit_3);
  }

  //if(To_rotate) Good_TOF1_intersect = _Good_TOF1(gap_to_fit_rotated-1, inter_tof1);
  if(To_rotate_pre_prunning || To_rotate_post_prunning) Good_TOF1_intersect = _Good_TOF1(gap_to_fit_rotated-1, inter_tof1);
  else Good_TOF1_intersect = _Good_TOF1(gap_to_fit-1,inter_tof1);
  ...
  x_int_TOF1[0] = intersectx1(a_lepton_fit_3, b_lepton_fit_3, R_TOF1);
  x_int_TOF1[1] = intersectx2(a_lepton_fit_3, b_lepton_fit_3, R_TOF1);
  y_int_TOF1[0] = y1_int(x_int_TOF1[0], a_lepton_fit_3, b_lepton_fit_3);
  y_int_TOF1[1] = y2_int(x_int_TOF1[1], a_lepton_fit_3, b_lepton_fit_3);
  ...
  dist1_TARGET_intersect[1] = 999.;       X_weights = 0.;

  dist1_TOF1_intersect[0] = 999.;
  dist1_TOF1_intersect[1] = 999.;
  ...
  for(int i=0; i<2; i++){
    ...
    if(To_rotate_pre_prunning || To_rotate_post_prunning){
      ...
      dist1_TOF1_intersect[i] = distance(x_int_TOF1[i], y_int_TOF1[i], Gap[TOF1_rotated[gap_to_fit-1]][1][0], Gap[TOF1_rotated[gap_to_fit-1]][1][1]);     
    }
    else{
      dist1_TDC_Gap_Fibers[i] = distance(x_int_TDC_Gap_Fibers[i], y_int_TDC_Gap_Fibers[i], Gap[gap_to_fit-1][1][0], Gap[gap_to_fit-1][1][1]);
      dist1_TDC_Gap_Fibers_SFT[i] = distance(x_int_TDC_Gap_Fibers_SFT[i], y_int_TDC_Gap_Fibers_SFT[i], Gap[gap_to_fit-1][1][0], Gap[gap_to_fit-1][1][1]);
      dist1_TARGET_intersect[i] = distance(x_int_TARGET[i], y_int_TARGET[i], Gap[gap_to_fit-1][1][0], Gap[gap_to_fit-1][1][1]);
      dist1_TOF1_intersect[i] = distance(x_int_TOF1[i], y_int_TOF1[i], Gap[gap_to_fit-1][1][0], Gap[gap_to_fit-1][1][1]);
    }
  }
  ...
  if(dist1_TOF1_intersect[0] < dist1_TOF1_intersect[1]){  // UNUSED
    x_TOF1_intersect = x_int_TOF1[0];
    y_TOF1_intersect = y_int_TOF1[0];
  }
  else if(dist1_TOF1_intersect[1] < dist1_TOF1_intersect[0]){  // UNUSED
    x_TOF1_intersect = x_int_TOF1[1];
    y_TOF1_intersect = y_int_TOF1[1];
  }
  else{
    if(batch==1) error_flag=0;      
    else{
  ...
  //cout << "PRU : " << vec_lepton_bars_pruned.size() << endl;
  //cout << "FIN : " << vec_lepton_bars_final.size() << endl;

  //cout << "SEB: " << vec_lepton_bars_final.size() << endl;

  if(vec_lepton_bars_final.size()>=lepton_cluster_size){
    //if(To_rotate){  // TO MOVE
    if(To_rotate_pre_prunning || To_rotate_post_prunning){  // TO MOVE
      ...
      vec_xx_int_TOF1.push_back(y_TOF1_intersect);
      vec_yy_int_TOF1.push_back(-x_TOF1_intersect);
    }
    else{   // TO MOVE
      ...
      vec_xx_int_TOF1.push_back(x_TOF1_intersect);
      vec_yy_int_TOF1.push_back(y_TOF1_intersect);
    }
  }
  else{
      vec_xx_int_TDC_Gap_Fibers.push_back(999.);
      vec_yy_int_TDC_Gap_Fibers.push_back(999.);

      vec_xx_int_TDC_Gap_Fibers_SFT.push_back(999.);
      vec_yy_int_TDC_Gap_Fibers_SFT.push_back(999.);

      vec_xx_int_TDC_TARGET.push_back(999.);
      vec_yy_int_TDC_TARGET.push_back(999.);

      vec_xx_int_TOF1.push_back(999.);
      vec_yy_int_TOF1.push_back(999.);  
  }

  //cout << endl;
  //cerr << "TDC Gap Fibers : " << vec_xx_int_TDC_Gap_Fibers[0] << "  " << vec_yy_int_TDC_Gap_Fibers[0] << endl;
  //cerr << "TDC Gap Fiber SFT : " << vec_xx_int_TDC_Gap_Fibers_SFT[0] << "  " << vec_yy_int_TDC_Gap_Fibers_SFT[0] << endl;
  //cerr << "TDC Target : " << vec_xx_int_TDC_TARGET[0] << "  " << vec_yy_int_TDC_TARGET[0] << endl;
  //cerr << "TOF1 : " << vec_xx_int_TOF1[0] << "  " << vec_yy_int_TOF1[0] << endl;
  //cout << endl;

  /// Check if lepton fit line intersects the proper TOF1 Gap
  //Good_TOF1_intersect = _Good_TOF1(gap_to_fit-1, vec_xx_int_TOF1, vec_yy_int_TOF1);
  ...
  if(To_rotate_pre_prunning || To_rotate_post_prunning) a_final_guide = a_lepton_fit_3;  // ????
  ...
  // use Delta_phi to RADIANS  MH -- Oct 14/17
  Delta_X = sqrt(pow(TARGET_Errors_X,2) + pow((sin(angle_final_guide*PI/180.)*R_TARGET*Delta_phi),2));
  Delta_Y = sqrt(pow(TARGET_Errors_X,2) + pow((cos(angle_final_guide*PI/180.)*R_TARGET*Delta_phi),2));
  ...
  if(To_rotate_pre_prunning || To_rotate_post_prunning){
    ...
    if(vec_kaon_bars.size()>0){
      ...
      //vec_xx_kstop_0.push_back(vec_xx_kaon_centroid[0]);
      //vec_yy_kstop_0.push_back(vec_yy_kaon_centroid[0]);
      //vec_xx_kstop_final.push_back(vec_xx_kaon_centroid[0]);
      //vec_yy_kstop_final.push_back(vec_yy_kaon_centroid[0]);
      }
        ...
        //vec_xx_kstop_0.push_back(Xloc[kaon_LG_max]);
        //vec_yy_kstop_0.push_back(Yloc[kaon_LG_max]);
        //vec_xx_kstop_final.push_back(Xloc[kaon_LG_max]);
        //vec_yy_kstop_final.push_back(Yloc[kaon_LG_max]);
      }
        ...
        //if(smallest_kaon_distance < 9.){
        //  vec_xx_kstop_0.push_back(vec_intersect_pr[0]);
        //  vec_yy_kstop_0.push_back(vec_intersect_pr[1]);
        //  //vec_xx_kstop_final.push_back(111.11);
        //  //vec_yy_kstop_final.push_back(111.11);
        //}
        //else{
        //  vec_xx_kstop_0.push_back(vec_xx_kaon_centroid[0]);
        //  vec_yy_kstop_0.push_back(vec_yy_kaon_centroid[0]);
  ...
  //cerr << "ALBANE TEST : " << a_fit_lepton_rotate << endl;
  //cerr << endl;
  ...
  //cout << "TEST KAON:  " << vec_kaon_bars.size() << endl;
  //cout << "TEST LEPTON:  " << vec_lepton_bars.size() << endl;
  //cout << "TEST LEPTON PRUNED:  " << vec_lepton_bars_pruned.size() << endl;

  //cout << "SIZE PRUNED : " << vec_lepton_bars_pruned.size() << endl;
  //cout << "SIZE FINAL : " << vec_lepton_bars_final.size() << endl;

/////////////////////////////////////////////////////////////////////////////////////////////
  //cout << "EMPTY LEPTON : " <<  empty_lepton3 << endl;

  if(!empty_lepton3){
    if(vec_kaon_bars.size()<=0){
      //cout << "I AM HERE ! NO ?" << endl;
      vec_xx_kstop_final.push_back(999.99);
      vec_yy_kstop_final.push_back(999.99);
    }
    else{
...
/*        if(empty_lepton3){
          //cout << "LA ! " << endl;
          //vec_xx_kstop_final.clear();
          //vec_yy_kstop_final.clear();
          vec_xx_kstop_final.push_back(vec_kaon_centroid_coordinates[0]);
          vec_yy_kstop_final.push_back(vec_kaon_centroid_coordinates[1]);
          kstop_centroid = true;
        }
*/
        if(vertical_kaon){
        ...
        vec_xx_kstop_0.push_back(Xloc[kaon_LG_max]);
        vec_yy_kstop_0.push_back(Yloc[kaon_LG_max]);
        vec_kstop_final.push_back(Xloc[kaon_LG_max]);
        vec_kstop_final.push_back(Yloc[kaon_LG_max]);
        vec_xx_kstop_final.push_back(vec_kstop_final[0]);
        vec_yy_kstop_final.push_back(vec_kstop_final[1]);
        kstop_centroid = true;
      }
    }
  }
  else{
    vec_fit_lines_intersect.push_back(999.);
    vec_fit_lines_intersect.push_back(999.);
    if(vec_kaon_bars.size()<=0){
      //cout << "I AM HERE ! NO ?" << endl;
      vec_xx_kstop_final.push_back(999.99);
    ...
    //cout << "SSSS ALBANE : " << vec_fit_lines_intersect.size() << endl;
    //cerr << vec_fit_lines_intersect[0] << "  " << vec_fit_lines_intersect[1] << endl;
...
/*  if(vec_kaon_bars.size()>0){
    if(vec_kaon_bars.size()<=kaon_cluster_size){
      ...
      if(vec_lepton_bars_pruned.size()==0){
        cout << "ICI !" << endl;
        //vec_xx_kstop_final.clear();
        //vec_yy_kstop_final.clear();
        vec_kstop_final.push_back(vec_kaon_centroid_coordinates[0]);
        vec_kstop_final.push_back(vec_kaon_centroid_coordinates[1]);
        vec_xx_kstop_final.push_back(vec_kaon_centroid_coordinates[0]);
        vec_yy_kstop_final.push_back(vec_kaon_centroid_coordinates[0]);    
        kstop_centroid = true; 
      }
      else{
      vec_kstop_final.push_back(999.99);
      vec_kstop_final.push_back(999.99);
      }
    }
  }
    ...
    if(vertical_kaon) {
      ...
      if(To_rotate){
        ...
        if(TO_ROTATE_KAON){
          ...
          else vec_fit_lines_intersect = _2lines_intersect(a_lepton_fit_3, b_lepton_fit_3, A_FIT_KAON, B_FIT_KAON);
        }
        else vec_fit_lines_intersect = _2lines_intersect(a_lepton_fit_3, b_lepton_fit_3, a_fit_kaon, b_fit_kaon);
      }
    }
...
*/
  ...
  if(!empty_lepton3){
    if(vec_kaon_bars.size() <= kaon_cluster_size){
      ...
      if(pow(vec_fit_lines_intersect[0],2)+pow(vec_fit_lines_intersect[1],2) > pow(R_TARGET,2)){
        vec_xx_kstop_final.push_back(vec_kaon_centroid_coordinates[0]);
        vec_yy_kstop_final.push_back(vec_kaon_centroid_coordinates[1]);
        kstop_centroid = true;     
      }
      else{
        //cout << "ON EST ICI MAINTENANT ! " << endl;
        smallest_kaon_distance = 999.;
        for(unsigned int j=0; j<vec_kaon_bars.size(); j++){

          dist_intersect_to_kaon = sqrt(pow((Xloc[vec_kaon_bars[j]]-vec_fit_lines_intersect[0]),2) + pow((Yloc[vec_kaon_bars[j]]-vec_fit_lines_intersect[1]),2));
          if(dist_intersect_to_kaon < smallest_kaon_distance){
            smallest_kaon_distance = dist_intersect_to_kaon;
            closest_kaon = vec_kaon_bars[j];
          }
        ...
        //if(dist_pruned || smallest_kaon_distance >= DISTANCE_MAX_TO_KAON_FINAL){
        if(smallest_kaon_distance >= DISTANCE_MAX_TO_KAON_FINAL){
          vec_xx_kstop_final.push_back(vec_kaon_centroid_coordinates[0]);
          vec_yy_kstop_final.push_back(vec_kaon_centroid_coordinates[1]);
          kstop_centroid = true;         
        }
        else{
          vec_xx_kstop_final.push_back(vec_fit_lines_intersect[0]);
          vec_yy_kstop_final.push_back(vec_fit_lines_intersect[1]);
          kstop_intersect = true;        
        }
        //else if(smallest_kaon_distance < DISTANCE_MAX_TO_KAON_FINAL){
          //cout << "BBBB" << endl;
          //vec_xx_kstop_final.push_back(vec_fit_lines_intersect[0]);
          //vec_yy_kstop_final.push_back(vec_fit_lines_intersect[1]);
          //kstop_intersect = true;
...
*//////////////////////////////////////////////////////////////////////////////////////////
  ...
  if(vec_fit_lines_intersect[0]*vec_fit_lines_intersect[0]+
     vec_fit_lines_intersect[1]*vec_fit_lines_intersect[1] > R_Cut*R_Cut){
    intersect_outside_target=true;
    vec_k_stop_coordinates.clear();
    vec_k_stop_coordinates.push_back(vec_kaon_centroid_coordinates[0]);
    vec_k_stop_coordinates.push_back(vec_kaon_centroid_coordinates[1]);  
  }
    ...
    // if(Switch_Printout !=0){
    //   cout << "###########################################################################" << endl;
    //   cout << "CENTROID FINAL" << endl;
    //   cerr << "X = " << vec_kaon_centroid_coordinates[0] << endl;
    //   cerr << "Y = " << vec_kaon_centroid_coordinates[1] << endl;
    //   cerr << endl;
    //   cerr << "KSTOP FINAL" << endl;
    //   cerr << "X = " << vec_xx_kstop_final[0] << endl;
    //   cerr << "Y = " << vec_yy_kstop_final[0] << endl;
    //   cout << "###########################################################################" << endl;
    //   cout << endl;
    // }
  ...
  if((vec_kaon_bars.size()>kaon_cluster_size) && (vec_lepton_bars.size()>lepton_cluster_size)){
    if(intersect_outside_target){
      //cout << endl;
      //cout << "//// INTERSECT IS OUTSIDE OF THE TARGET !" << "   " << ievt << endl;
      //cout << endl;
    }
  }
...
/*  if(To_rotate){
    for(unsigned int i=0; vec_xx_kstop_0.size(); i++){
      vec_xx_kaon_stop.push_back(vec_yy_kstop_0[i]);
      vec_yy_kaon_stop.push_back(-vec_xx_kstop_0[i]);
    }
  }
  else{
    for(unsigned int i=0; vec_xx_kstop_0.size(); i++){
      vec_xx_kaon_stop.push_back(vec_xx_kstop_0[i]);
      vec_yy_kaon_stop.push_back(vec_yy_kstop_0[i]);
    }   
  }
*/
  //return;
  ...
  //return;
  i_kaon_bar = 999;
  //cout << "/////// -> " << vec_xx_kstop_final.size() << "  " << vec_yy_kstop_final.size() << endl;
  //return;
  //i_kaon_bar = kaon_fiber(vec_k_stop_coordinates[0],vec_k_stop_coordinates[1]);
  //if(abs(vec_k_stop_coordinates[0])>50 || abs(vec_k_stop_coordinates[1])>50) i_kaon_bar = 999;
  i_kaon_bar = kaon_fiber(vec_xx_kstop_final[0],vec_yy_kstop_final[0]);
  //return;
  if(abs(vec_xx_kstop_final[0])>50 || abs(vec_yy_kstop_final[0])>50) i_kaon_bar = 999;
  //return;
  ...
  if (counter_TDC_kaon > 0) Average_TDC_kaon = sum_TDC_kaon/counter_TDC_kaon;
    ...
    vector<int> vec_leptons;
    //if(To_rotate){
    if(To_rotate_pre_prunning || To_rotate_post_prunning){
      for(unsigned int i=0; i<vec_lepton_bars_rotate.size(); i++){
        vec_leptons.push_back(vec_lepton_bars_rotate[i]);
      }
    }
    else{
      for(unsigned int i=0; i<vec_lepton_bars_final.size(); i++){
        vec_leptons.push_back(vec_lepton_bars_final[i]);
      }   
    }
    ...
    vector<double> v_intersect_SFT;
    vector<double> v_intersect_TARGET;
    double sslope = 999.99;
    double sb = 999.99;

    if(vec_leptons.size()<2 || (vec_leptons.size()==2 && angle_final_guide==999.)){
      vec_test = lineLeptonToTOF1(vec_leptons, gap_to_fit, vec_xx_kstop_final[0], vec_yy_kstop_final[0]);
      line_test = new TLine(vec_test[0], vec_test[1], vec_test[2], vec_test[3]);
      b_bbar = true;
    }

    if(b_bbar){
      vec_xx_TOF1_closest.push_back(Gap[gap_to_fit-1][2][0]);
      vec_yy_TOF1_closest.push_back(Gap[gap_to_fit-1][2][1]);

      v_slope = get_slope(vec_test[0], vec_test[1], vec_test[2], vec_test[3]);
      sslope = v_slope[0];
      sb = v_slope[1];

      v_intersect_SFT = intersect_circle(sslope, sb, R_SFT_L1, vec_xx_TOF1_closest[0], vec_yy_TOF1_closest[0]);
      v_intersect_TARGET = intersect_circle(sslope, sb, R_TARGET, vec_xx_TOF1_closest[0], vec_yy_TOF1_closest[0]);

      vec_xx_int_TDC_Gap_Fibers_SFT[0] = v_intersect_SFT[0];
      vec_yy_int_TDC_Gap_Fibers_SFT[0] = v_intersect_SFT[1];
      vec_xx_int_TDC_TARGET[0] = v_intersect_TARGET[0];
      vec_yy_int_TDC_TARGET[0] = v_intersect_TARGET[1];

      angle_final_guide = angle_calculation(sslope, vec_test[0], vec_test[1], vec_test[2], vec_test[3]);
      Delta_phi_deg = 99.99;
      Chis_lepton_fit_3 = -1.;
      ndf_lepton_fit_3 = 1;
    }
    else{
      vec_xx_TOF1_closest.push_back(Gap[gap_to_fit-1][selected_TDC_selected][0]);
      vec_yy_TOF1_closest.push_back(Gap[gap_to_fit-1][selected_TDC_selected][1]);     
    }
  ...
  if(Switch_Printout != 0){
    ...
    cout << "  " << endl;
    cout << "OLD" << endl;
    printf("                    Fiber  ADC-Ped   TDC   ADC(HG)  ADC(LG)\n");
    printf("LG ADC 1st Max.  :  %4d    %4d    %4d    %4d     %4d\n",index_max1, ADC_Low_TARGET[index_max1] + TARGET_ADC_Thr_LG_Offset, TDC_LG_max, ADC_High_TARGET[index_max1], ADC_Low_TARGET[index_max1]);
    printf("LG ADC 2nd Max.  :  %4d    %4d    %4d    %4d     %4d\n",index_max2, ADC_Low_TARGET[index_max2] + TARGET_ADC_Thr_LG_Offset, TDC_LG_max2, ADC_High_TARGET[index_max2], ADC_Low_TARGET[index_max2]);
      ...
      cout << "Fiber  HG-Ped   LG-Ped   T[0]-Av   T[1]-Av   T[2]-Av   T[3]-Av   ADC(HG)   ADC(LG)      TDC    Thr(HG)   Thr(LG)   HG/LG" << endl;
        for(Int_t jj=0; jj<256; jj++){
          if(ADC_High_TARGET[jj]>0 || (ADC_Low_TARGET[jj]>0 && Switch==1)){
            for(int k=0; k<6; k++){
              //if(tdc_le_target[jj][k]>=TDC_Thr_min && tdc_le_target[jj][k]<=TDC_Thr_max){
    ...
    cerr << "" << endl;
    cerr << "" << endl;
    cerr << "Final Angle (deg.) = " << angle_final_guide << " +- " << Delta_phi_deg << "  ( GUIDE )" << endl;
    cerr << "Chi Square = " << Chis_lepton_fit_3 << endl;
    cerr << "NdF = " << ndf_lepton_fit_3 << endl;
    cerr << "Reduced Chi Square = " << Chis_lepton_fit_3/ndf_lepton_fit_3 << endl;
    cerr << "" << endl;
    cerr << "" << endl;

    if(dist_to_k_stop != -10000){
      cerr << "Distance from K-stop to best fit line = " << dist_to_k_stop << endl << endl;
    }
    else{
      cerr << "No k-stop centroid." << endl << endl;
    }

    if(gap_to_fit==6 || gap_to_fit==12){
      cerr << "Intersect with SFT Layer 1:  " << "x = " << -y_TDC_Gap_Fibers_SFT_intersect1 << "   " << "y = " << x_TDC_Gap_Fibers_SFT_intersect1 << endl;
      cerr << "Intersect with TOF1:  " << "x = " << -y_TDC_Gap_Fibers_intersect1 << "   " << "y = " << x_TDC_Gap_Fibers_intersect1 << endl;
    }
    else{
      cerr << "Intersect with SFT Layer 1:  " << "x = " << x_TDC_Gap_Fibers_SFT_intersect1 << "   " << "y = " << y_TDC_Gap_Fibers_SFT_intersect1 << endl;
      cerr << "Intersect with TOF1:  " << "x = " << x_TDC_Gap_Fibers_intersect1 << "   " << "y = " << y_TDC_Gap_Fibers_intersect1 << endl;
    }
    cerr << endl;

    if(gap_to_fit==6 || gap_to_fit==12){
      cerr << "X_TOF1 = " << Gap[gap_to_fit-1][selected_TDC_selected][0] << "   ;   Y_TOF1 = " << Gap[gap_to_fit-1][selected_TDC_selected][1] << endl;
      Yth_TOF1 = a_lepton_fit_3*Gap[gap_to_fit-1][selected_TDC_selected][0] + b_lepton_fit_3;
      cerr << "Calculated Y_TOF1 = " << Yth_TOF1 << endl;
      cerr << "Delta_Y = " << Gap[gap_to_fit-1][selected_TDC_selected][1] - Yth_TOF1 << endl;
    }
    else{
      cerr << endl;
      cerr << "gap_to_fit : " << gap_to_fit << endl;
      cerr << "selected_TDC_selected : " << selected_TDC_selected << endl;
...
/*  cout << "//////////////////////////////////////////////////////////////////////////" << endl;
  ...
  /////print1(NoFile, ParsTarg1, nentries, ievt, enable_cout, Switch, n_bar_selected);
*/
  ///////////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  //if(TO_ROTATE_KAON){
    //cout << endl;
    //cout << "TO DRAW : " << to_draw << endl;
    //cout << "TO ROTATE KAON : " << TO_ROTATE_KAON << "   " << ievt << endl;
    //cout << endl;
  //}
...
*/
    ...
    sprintf(str_pruning,"Data Pruning  |  Run %d  --  Event %d",Run_Number,ievt);
    ...
    TH2F *h_pruning = new TH2F("Data Pruning", str_pruning, 500, -50, 50, 500, -50, 50);
    ...
    gr_lepton_pr->SetMarkerStyle(21);
    gr_lepton_pr->SetMarkerColor(2);
    gr_lepton_pr->SetMarkerSize(0.8);  
    ...
    if(b_bbar) tex_ChiS->SetTextColor(2);
...
/*    if(To_rotate){
      sprintf(X_SFT_String,"X(41.6) = %4.2f", y_TDC_Gap_Fibers_SFT_intersect1);    //  X' = Y  ;  Y' = -X
      sprintf(Y_SFT_String,"Y(41.6) = %4.2f", -x_TDC_Gap_Fibers_SFT_intersect1);     //  X' = Y  ;  Y' = -X
      ...
      sprintf(X_SFT_String,"X(41.6) = %4.2f", x_TDC_Gap_Fibers_SFT_intersect1);
      sprintf(Y_SFT_String,"Y(41.6) = %4.2f", y_TDC_Gap_Fibers_SFT_intersect1);
...
*///    if(To_rotate){
      ...
      //cout << "POE X : " << vec_xx_int_TDC_Gap_Fibers_SFT[0] << endl;
      //cout << "POE Y : " << vec_yy_int_TDC_Gap_Fibers_SFT[0] << endl;

      sprintf(X_SFT_String,"X(41.6) = %4.2f", vec_xx_int_TDC_Gap_Fibers_SFT[0]);    //  X' = Y  ;  Y' = -X
      sprintf(Y_SFT_String,"Y(41.6) = %4.2f", vec_yy_int_TDC_Gap_Fibers_SFT[0]);     //  X' = Y  ;  Y' = -X
      sprintf(X_TOF1_String,"X(47.1) = %4.2f", vec_xx_int_TDC_Gap_Fibers[0]);     //  X' = Y  ;  Y' = -X
      sprintf(Y_TOF1_String,"Y(47.1) = %4.2f", vec_yy_int_TDC_Gap_Fibers[0]);      //  X' = Y  ;  Y' = -X
      sprintf(X_TARGET_String,"X(29) = %4.2f", vec_xx_int_TDC_TARGET[0]);               //  X' = Y  ;  Y' = -X
      sprintf(Y_TARGET_String,"Y(29) = %4.2f", vec_yy_int_TDC_TARGET[0]);               //  X' = Y  ;  Y' = -X
//    }
/*    else{
      sprintf(X_SFT_String,"X(41.6) = %4.2f", x_TDC_Gap_Fibers_SFT_intersect1);
      sprintf(Y_SFT_String,"Y(41.6) = %4.2f", y_TDC_Gap_Fibers_SFT_intersect1);
...
*/
    ...
    sprintf(str_Kstop_X,"X_{Ks} = %5.2f",vec_xx_kstop_final[0]);
    sprintf(str_Kstop_Y,"Y_{Ks} = %5.2f",vec_yy_kstop_final[0]);
    ...
    TGraph *gr_int_TOF1 = new TGraph(vec_xx_int_TOF1.size(),&vec_xx_int_TOF1[0],&vec_yy_int_TOF1[0]);
    ...
    TGraph *gr_kaon_stop = new TGraph(vec_xx_kstop_final.size(),&vec_xx_kstop_final[0],&vec_yy_kstop_final[0]);

    //graph_kstop_0 = new TGraph(vec_xx_kstop_0.size(),&vec_xx_kstop_0[0],&vec_yy_kstop_0[0]);

    //if(To_rotate) gr_kaon_stop = new TGraph(vec_xx_kaon_stop.size(),&vec_yy_kaon_stop[0],-&vec_xx_kaon_stop[0]);
    //else gr_kaon_stop = new TGraph(vec_xx_kaon_stop.size(),&vec_xx_kaon_stop[0],&vec_yy_kaon_stop[0]);
...
/*    if(vec_xx_kaon.size()< kaon_cluster_size || intersect_outside_target){
      cout << "RRRRR 1 " << endl;
      gr_kaon_stop->SetMarkerStyle(20);
      gr_kaon_stop->SetMarkerColor(1);
      gr_kaon_stop->SetMarkerSize(0.9);
...
/*    vector<int> vec_leptons;
    ...
    if(b_bbar){
      vec_xx_TOF1_closest.push_back(Gap[gap_to_fit-1][1][0]);
      vec_yy_TOF1_closest.push_back(Gap[gap_to_fit-1][1][1]);

      sslope = get_slope(vec_test[0], vec_test[1], vec_test[2], vec_test[3]);
      angle_final_guide = angle_calculation(sslope, vec_test[0], vec_test[1], vec_test[2], vec_test[3]);
    }
    else{
      vec_xx_TOF1_closest.push_back(Gap[gap_to_fit-1][selected_TDC_selected][0]);
...
*/
    ...
    To_rotate = false;
    if(To_rotate_pre_prunning || To_rotate_post_prunning) To_rotate = true;
    ...
    /// TARGET2 PLOTTING
    c2->cd(1);
    Subplot_1(ch_Title1, ch_ADC_cut_TARGET, tex_Legend_TARGET, tex_event_TARGET,
    ...
    c2->cd(2);
    Subplot_2(ch_Title2, ch_ADC_and_TDC_cut, tex_Legend_TARGET, tex_event_TARGET,
              ADC_High_TARGET, marker_ADC_TARGET,
              ADC_Low_TARGET, TDC_min_TARGET,
              Switch, has_TDC_hit, palette_TARGET, tex_palette_TARGET,
              tex_palette_TARGET_scale, max_index,
...
/*  if(vec_leptons.size()==0){
    ...
    vec_test = testtt(vec_leptons, gap_to_fit, vec_xx_kstop_final[0], vec_yy_kstop_final[0]);
    line_test = new TLine(vec_test[0], vec_test[1], vec_test[2], vec_test[3]);
    c2->cd(6);
    line_test->Draw();
    ...
    cout << "TEST " << endl;
    cout << "Gap to fit " << gap_to_fit << endl;
    cout << vec_test[0] << endl;
    cout << vec_test[1] << endl;
    cout << vec_test[2] << endl;
    cout << vec_test[3] << endl;
  }

  if(vec_leptons.size()==1){
...
/*  vector<double> vec_test;
  TLine *line_test;
  bool bar_are_neighbours = false;

  if(vec_leptons.size()==0){
    vec_test = lineLeptonToTOF1(gap_to_fit, vec_xx_kstop_final[0], vec_yy_kstop_final[0], 0);
    line_test = new TLine(vec_test[0], vec_test[1], vec_test[2], vec_test[3]);
    c2->cd(6);
    line_test->Draw();
  }
  if(vec_leptons.size()==1){
    if(To_rotate){
      //cout << "TTTTT : " << To_rotate << "  " << vec_lepton_bars_rotate[0] << endl;
      vec_test = lineLeptonToTOF1(gap_to_fit, Xloc[vec_lepton_bars_rotate[0]], Yloc[vec_lepton_bars_rotate[0]], 1);
      line_test = new TLine(vec_test[0], vec_test[1], vec_test[2], vec_test[3]);
      c2->cd(6);
      line_test->Draw();
      ...
      //cout << "TTTTT : " << To_rotate << "  " << vec_lepton_bars_final[0] << endl;
      vec_test = lineLeptonToTOF1(gap_to_fit, Xloc[vec_lepton_bars_final[0]], Yloc[vec_lepton_bars_final[0]], 1);
      line_test = new TLine(vec_test[0], vec_test[1], vec_test[2], vec_test[3]);
      c2->cd(6);
      line_test->Draw();
    }
...
*//*  if(lepton_counter==2){
    if(To_rotate){
      cout << "BAR 1 : " << vec_lepton_bars_rotate[0] << endl;
      cout << "BAR 2 : " << vec_lepton_bars_rotate[1] << endl;
      bar_are_neighbours = areneighbours(vec_lepton_bars_rotate[0], vec_lepton_bars_rotate[1]);

      if(bar_are_neighbours){

      }
    }
    else{
      cout << "BAR 1 : " << vec_lepton_bars_final[0] << endl;
      cout << "BAR 2 : " << vec_lepton_bars_final[1] << endl;
      bar_are_neighbours = areneighbours(vec_lepton_bars_final[0], vec_lepton_bars_final[1]);
     }
  }
*/
  //return;
  ...
  //cout << endl;
    ...
    graph_kaon_pr->Draw("sameP");
    graph_lepton_pr->Draw("sameP");
    if(vec_kaon_bars.size()>kaon_cluster_size) graph_kaon_centroid_0->Draw("sameP");
    if(vec_kaon_bars.size()>0) graph_kstop_0->Draw("sameP");
    //pruning_line->Draw("same");
    //if(repruned_flag) para_pruning->Draw("same");

    c_pruning->cd(2);
    h_Pruning[0]->Draw();
    ell_Target->Draw("same");
    graph_kaon_pr->Draw("sameP");
    //graph_lepton_pr2->Draw("sameP");
    graph_lepton_pr->Draw("sameP");
    pruning_line->Draw("same");
    if(repruned_flag) para_pruning->Draw("same");
    graph_kstop_0->Draw("sameP");
    if(vec_kaon_bars.size()>kaon_cluster_size){
      graph_kaon_centroid_0->Draw("sameP");
    }
    gr_pruning_line_plot->Draw("sameP");

    c_pruning->cd(3);
    h_Pruning[0]->Draw();
    ell_Target->Draw("same");
    //graph_kaon_pr3->Draw("sameP");
    graph_lepton_pr3->Draw("sameP");
    graph_kaon_pr3->Draw("sameP");
    //graph_lepton_pr4->Draw("sameP");
    ...
    if(vec_kaon_bars.size()>0){
      //graph_kstop_0->Draw("sameP");
      gr_pruning_area->Draw("samel");     
      //gr_pruning_polygon_plot->Draw("sameP");
    }
    graph_kstop_0->Draw("sameP");
    gr_pruning_polygon_plot->Draw("sameP");
    ...
    ell_Target->Draw("same");
    graph_kaon_pr3->Draw("sameP");
    gr_pruning_polygon->Draw("sameP");
    //graph_lepton_pr->Draw("sameP");
    //if(vec_kaon_bars.size()>0) graph_kstop_0->Draw("sameP");
   

    //c_pruning->cd()->Range(-50,-50,50,50);
    //Subplot_pruning(h_pruning, A1,
    //          A2, gr_Target_Center,
    //          ell, ell_Target,
    //          ell_L1,
    //          To_rotate,
    //          gr_lepton_pr,
    //          graph_kaon, graph_kaon_rotate);
              ...
              //to_draw, best_kaon_fit_rotate);
    //perp2->SetLineWidth(2);
    //perp2->Draw("same");
    //perp3->SetLineWidth(2);
    //perp3->Draw("same");
    
    //L_TEST->SetLineWidth(2);
    //L_TEST->SetLineColor(6);
    //L_TEST->Draw("same");
    
    //para1->SetLineWidth(2);
    //para1->SetLineColor(1);
    //para1->Draw("same");
...
/*  cout << endl;
  cout << "TEST : " << endl;
  cout << "size :" << vec_xx_kstop_final.size() << "  " << vec_xx_kstop_final.size() << endl;
  for(unsigned int i=0; i<vec_xx_kstop_final.size(); i++) cerr << "x = " << vec_xx_kstop_final[i] << "  ";
  cout << endl;
  for(unsigned int i=0; i<vec_yy_kstop_final.size(); i++) cerr << "y = " << vec_yy_kstop_final[i] << "  ";
  cout << endl;
  cout << endl;
*/

  xx_int_TDC_TARGET = vec_xx_int_TDC_TARGET[0];
  yy_int_TDC_TARGET = vec_yy_int_TDC_TARGET[0];

  xx_kaon_centroid_coordinates = vec_kaon_centroid_coordinates[0];
  yy_kaon_centroid_coordinates = vec_kaon_centroid_coordinates[1];

  xx_fit_lines_intersect = vec_fit_lines_intersect[0];
  yy_fit_lines_intersect = vec_fit_lines_intersect[1];

  xx_kstop_final = vec_xx_kstop_final[0];
  yy_kstop_final = vec_yy_kstop_final[0];
  R_Kstop = sqrt(xx_kstop_final*xx_kstop_final + yy_kstop_final*yy_kstop_final);

  kaon_bar_size = vec_kaon_bars.size();

  Ck_size = vec_Ck.size();
  Cpi_size = vec_Cpi.size();

  reduced_Chis = Chis_lepton_fit_3/ndf_lepton_fit_3;  

  reduced_kaon_Chis = Chis_kaon/ndf_kaon;
...
/*  cout << "ALBANE : " << vec_kaon_bars.size() << endl;
  for(unsigned int i=0; i<vec_kaon_bars.size(); i++){
    cout << "## " << vec_kaon_bars[i] << endl;
  }
*/
  //line_test->Draw();
  ...
  if(print_output != 0){
    ...
    cout << setw(7) << setprecision(2) << vec_xx_kstop_final[0] << " ";             // 21
    cout << setw(7) << setprecision(2) << vec_yy_kstop_final[0] << " ";
    cout << setw(7) << setprecision(2) << R_Kstop << " ";             // 22
    cout << setw(3) << i_kaon_bar << " ";
    cout << setw(3) << vec_kaon_bars.size() << " ";
    cout << setw(5) << setprecision(2) << Chis_kaon/ndf_kaon << " ";
    ...
    cout << setw(1) << setprecision(0) << has_edge_bars << " ";
    cout << setw(1) << setprecision(0) << Good_TOF1_intersect << " ";
    cout << endl;
  ...
  if(batch==1 && (batch_flag== 0 || batch_flag==2 || batch_flag==3)){ // {print_output
    fout << fixed;
    fout << setw(4) << Run_Number << " ";                                               //  1
    fout << setw(7) << ievt << " ";                                                     //  2
    fout << setw(1) << error_flag << " ";                                               //  3
    fout << setw(2) << Event_tag[0] << " ";                                             //  4
    fout << setw(2) << gap_to_fit << " ";                                               //  5
    fout << setw(2) << selected_TOF2 << " ";                                            //  6
    fout << setw(6) << setprecision(2) << angle_final_guide << " ";                     //  7
    fout << setw(6) << setprecision(2) << Delta_phi_deg << " ";                         //  8
    fout << setw(6) << setprecision(2) << Delta_X << " ";                               //  9
    fout << setw(6) << setprecision(2) << Delta_Y << " ";                               // 10
    fout << setw(7) << setprecision(2) << Chis_lepton_fit_3 << " ";                     // 11
    fout << setw(3) << setprecision(0) << ndf_lepton_fit_3 << " ";                      // 12
    fout << setw(6) << setprecision(2) << Chis_lepton_fit_3/ndf_lepton_fit_3 << " ";    // 13
    fout << setw(3) << lepton_counter << " ";                                          // 14
    fout << setw(7) << setprecision(2) << vec_xx_int_TDC_TARGET[0] << " ";              // 15
    fout << setw(7) << setprecision(2) << vec_yy_int_TDC_TARGET[0] << " ";             // 16
    fout << setw(7) << setprecision(2) << vec_kaon_centroid_coordinates[0] << " ";      // 17
    fout << setw(7) << setprecision(2) << vec_kaon_centroid_coordinates[1] << " ";      // 18
    fout << setw(7) << setprecision(2) << vec_fit_lines_intersect[0] << " ";            // 19
    fout << setw(7) << setprecision(2) << vec_fit_lines_intersect[1] << " ";            // 20
    //fout << setw(7) << setprecision(2) << vec_k_stop_coordinates[0] << " ";          
    //fout << setw(7) << setprecision(2) << vec_k_stop_coordinates[1] << " ";           
    fout << setw(7) << setprecision(2) << vec_xx_kstop_final[0] << " ";                 // 21
    fout << setw(7) << setprecision(2) << vec_yy_kstop_final[0] << " ";                 // 22
    fout << setw(7) << setprecision(2) << R_Kstop << " ";             // 22
    fout << setw(3) << i_kaon_bar << " ";                                               // 23
    fout << setw(3) << vec_kaon_bars.size() << " ";                                     // 24
    fout << setw(6) << setprecision(2) << Chis_kaon/ndf_kaon << " ";                    // 25
    fout << setw(3) << vec_Ck.size() << " ";                                            // 26
    fout << setw(3) << vec_Cpi.size() << " ";                                           // 27
    fout << setw(7) << setprecision(2) << length_in_target << " ";                      // 28
    fout << setw(7) << setprecision(2) << C2X_centroid << " ";                          // 29
    fout << setw(6) << setprecision(1) << TDC_diff << " ";                              // 30
    fout << setw(6) <<  sum_ADC_HG_lepton << " ";                                       // 31
    fout << setw(7) << setprecision(2) << Average_TDC_lepton << " ";                    // 32
    fout << setw(7) << setprecision(2) << Average_TDC_kaon << " ";                      // 33
    fout << setw(1) << setprecision(0) << has_edge_bars << " ";                         // 34
    fout << setw(1) << setprecision(0) << Good_TOF1_intersect <<" ";                    // 35
    fout << endl;
    fout.close();
    ...
    fout_csv << setprecision(2) << angle_final_guide << ",";                     //  7
    fout_csv << setprecision(2) << Delta_phi_deg << ",";                         //  8
    fout_csv << setprecision(2) << Delta_X << ",";                               //  9
    fout_csv << setprecision(2) << Delta_Y << ",";                               // 10
    fout_csv << setprecision(2) << Chis_lepton_fit_3 << ",";                     // 11
    fout_csv << setprecision(0) << ndf_lepton_fit_3 << ",";                      // 12
    fout_csv << setprecision(2) << Chis_lepton_fit_3/ndf_lepton_fit_3 << ",";    // 13
    fout_csv << lepton_counter << ",";                                           // 14
    fout_csv << setprecision(2) << vec_xx_int_TDC_TARGET[0] << ",";              // 15
    fout_csv << setprecision(2) << vec_yy_int_TDC_TARGET[0] << "," ;             // 16
    fout_csv << setprecision(2) << vec_kaon_centroid_coordinates[0] << ",";      // 17
    fout_csv << setprecision(2) << vec_kaon_centroid_coordinates[1] << ",";      // 18
    fout_csv << setprecision(2) << vec_fit_lines_intersect[0] << ",";            // 19
    fout_csv << setprecision(2) << vec_fit_lines_intersect[1] << ",";            // 20
    //fout_csv << setprecision(2) << vec_k_stop_coordinates[0] << ",";          
    //fout_csv << setprecision(2) << vec_k_stop_coordinates[1] << ",";           
    fout_csv << setprecision(2) << vec_xx_kstop_final[0] << ",";                 // 21
    fout_csv << setprecision(2) << vec_yy_kstop_final[0] << ",";                 // 22
    fout_csv << setw(7) << setprecision(2) << R_Kstop << ",";             // 22
    fout_csv << i_kaon_bar << ",";                                               // 23
    fout_csv << vec_kaon_bars.size() << ",";                                     // 24
    fout_csv << setprecision(2) << Chis_kaon/ndf_kaon << ",";                    // 25
    ...
    fout_csv << setprecision(0) << has_edge_bars << ",";                         // 34
    fout_csv << setprecision(0) << Good_TOF1_intersect;                          // 35
    fout_csv << endl;
    fout_csv.close(); 
  }
  ...
  //int t_run_number, t_ievt, t_evt_flag;
